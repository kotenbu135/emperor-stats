...
# `号年` は 2026-08-17 に足した（北魏ブロック）。魏書は**自ら帝号を称した側**の建元を
# 「號年〈元号〉」と書く（「共推長廣王曄為主，大赦所部，**號年建明**」「自稱尊號，**號年天啟**」）。
# `年号` の語順が逆で、`号` 単独では `号建明` にならないため隣接形から漏れていた。
# **建てた側だけに当たる**（捨てた側は同じ紀の「改建明二年為普泰元年」＝ RECAST 側に立つ）
ERA_ANCHOR_PREFIX = ("改元", "建元", "改年", "年号", "号年", "立年", "号", "曰")
ERA_ANCHOR_SUFFIX = ("元年",)
# 「立年」は旧唐書が自ら帝号を称した側の建元に使う形（「贼窃号燕国，立年圣武」＝安禄山。
# 北魏の「號年」と同じ向きの穴で、2026-08-20 の唐末群雄ブロックで見つけた）
...
# **「建元为〈建てる側〉」も同じ枝で拾う**（2026-08-20 の隋末群雄ブロック。旧唐書・隋書の
# 列伝は僭称の建元をこの形でしか書かず、5人が値は正しいのに落ちた）。
# 建元は捨てる側を挟まないので 0字＝直後の 为 だけを許す（「建康元年为」型に当たらない）。
ERA_ANCHOR_RECAST = re.compile(r"(?:改[^，。；、]{0,6}|建元)[为為]$")
...
ERA_NAME_WITNESS_IN = {
    ...
    "tangmo-anqingxu.eraChangeCount.e001": (
        "book:daizhigev20/史藏/正史/新唐书.txt#18306-18320",
        "載初。新唐書 巻二百二十五上 逆臣上「既袭伪位，改载初元年」。キャッシュの旧唐書抜粋に改元の語が無い"),
    "tangmo-anqingxu.eraChangeCount.e002": (
        "book:daizhigev20/史藏/编年/资治通鉴.txt#22347-22380",
        "天成。資治通鑑 巻二百二十（至徳二年）「改鄴郡为安成府，改元天成」。新唐書は天和・薊門紀乱は至成と割れ、考異が実録の天成に従う（conflicts に記録）"),
    "tangmo-shisiming.eraChangeCount.e001": (
        "book:daizhigev20/史藏/编年/资治通鉴.txt#22453-22480",
        "順天。資治通鑑 巻二百二十一（乾元二年四月）「史思明自称大燕皇帝，改元顺天」"),
    "tangmo-shisiming.eraChangeCount.e002": (
        "book:daizhigev20/史藏/编年/资治通鉴.txt#22562-22570",
        "応天。資治通鑑 巻二百二十二（上元二年正月癸卯）「史思明改元应天」。順天より前の周王期の建元"),
    "tangmo-shichaoyi.eraChangeCount.e001": (
        "book:daizhigev20/史藏/编年/资治通鉴.txt#22561-22580",
        "顕聖。資治通鑑 巻二百二十二（上元二年三月戊戌）「朝义即皇帝位，改元显圣」"),
    "tangmo-li-yun.eraChangeCount.e001": (
        "book:daizhigev20/史藏/正史/旧唐书.txt#1675-1685",
        "建貞。旧唐書 僖宗紀（光啓二年五月庚辰）「襄王僭即皇帝位，年号建贞」。キャッシュの僖宗紀抜粋には入っていない"),
    # 前漢 — 本紀と列伝で元号名の字数が割れる（2026-08-20 漢系ブロック）
    "han-aidi.eraChangeCount.e002": (
        "book:china-history/汉书/传/第七十五章-王莽传上-原文.html#23-23",
        "太初元將。哀帝紀・天文志はいずれも2字形「以建平二年为太初元年」で書き、4字形は王莽伝上"
        "「更为太初元将元年」だけが持つ（宋祁の校勘が2字形を後人の脱字と説明する。conflicts に対立を記録）"),
    # 十六国 — 載記の抜粋キャッシュに改元条が入っていない短命帝（2026-08-20 十六国ブロック）
    "houyan-murongxiang.eraChangeCount.e001": (
        "book:daizhigev20/史藏/编年/资治通鉴.txt#9755-9770",
        "建始。晋書載記は「详僭称尊号，置百官，改年号」と名を書かない。資治通鑑 隆安元年"
        "「乃即皇帝位，改元建始」が名を持つ（慕容盛の建始〔401〕・趙王倫の建始と同名なので行範囲で絞る）"),
    "xiyan-murongchong.eraChangeCount.e001": (
        "book:daizhigev20/史藏/编年/资治通鉴.txt#9337-9345",
        "更始。キャッシュの晋書抜粋に改元条が無い。資治通鑑 太元十年「慕容冲即皇帝位于阿房，改元更始」"
        "（漢更始帝の更始と同名なので行範囲で絞る）"),
    # 南北朝 — 改元条が名を挙げない・本紀が立たない側（2026-08-20 南北朝ブロック）
    "liang-yuzhangwang.eraChangeCount.e001": (
        "book:china-history/南史/原文版南史/卷八-原文.html#40-46",
        "天正。キャッシュの梁書は「大赦改年」と名を書かない。南史 巻八「栋即位，改元天正」が名を持つ"),
    "houliang-mingdi.eraChangeCount.e001": (
        "book:daizhigev20/史藏/正史/周书.txt#4265-4275",
        "天保。後梁は本紀が立たず、キャッシュの周書 巻48 抜粋に嗣位の条が無い。同巻蕭詧伝末尾"
        "「高祖又命其太子岿嗣位，年号天保」が名を持つ（北斉文宣帝の天保と同名なので行範囲で絞る）"),
    # 晋 — 改元条が名を挙げない・帝紀が立たない側（2026-08-20 晋・三国ブロック）
    "jin-huidi.eraChangeCount.e003": (
        "book:daizhigev20/史藏/正史/晋书.txt#3305-3320",
        "元康。永平元年三月壬辰の改元条は「大赦，改元」で名を書かず、一年再改元の年は後の元号で"
        "標記する書式のため帝紀に「元康元年」の見出しも立たない。五行志「惠帝元康元年十二月辛酉，"
        "京都地震」（本紀の同年の地震・楚王瑋の事件と同一年）が名を持つ"),
    "jin-simalun.eraChangeCount.e001": (
        "book:daizhigev20/史藏/正史/晋书.txt#5050-5065",
        "建始。趙王倫は帝紀が立たず、キャッシュの恵帝紀は「赵王伦篡帝位」とだけ書く。"
        "改元条は趙王倫伝「乃僭即帝位，大赦，改元建始」。同名の建始（慕容詳397ほか）が"
        "他書に在るので行範囲まで絞る"),
    # 底本の脱字 — china-history 金史 巻16（宣宗紀下）は年首見出しを「元光年」と
    # 1字落として刻み（PUA ではなく字並びの欠落・claims 台帳 2026-08-20 が実測）、
    # 本紀の改元条「以彗星见，改元，大赦」は名を書かない。名を持つ改元記事は天文志の側
    "jin-xuanzong.eraChangeCount.e003": (
        "book:daizhigev20/史藏/正史/金史.txt#1550-1560",
        "元光。金史 天文志「宜改元修政以消天变。于是改是年为元光元年」（興定六年八月の彗星）。"
        "巻16 の年首「元光年春正月」は底本の脱字で隣接形に掛からない"),
...
COURTESY_ALLOW = {
    "jin-simalun":
        "**本人のキャッシュ（晉書 惠帝紀の篡位の条）の外に名乗りが在る** — キャッシュは"
        "「永宁元年春正月乙丑，赵王伦篡帝位。」から始まる地の文で、彼自身の書き出しを含まない。"
        "字を掲げるのは晉書 卷五十九 列傳第二十九（daizhigev20/史藏/正史/晋书.txt L5050）"
        "「赵王伦，字子彝，宣帝第九子也，母曰柏夫人。」。**巻は手で書かず "
        "`book_volumes.daizhige_spans` から引いた**（卷59 = L5026〜L5120）。"
        "**キャッシュを晉書 卷五十九まで広げれば免除は消せる**（2026-08-16・字ブロック4）",
    "liang-xiaoyuanming":
        "**本人のキャッシュ（梁書 敬帝紀の即位の条）の外に名乗りが在る** — キャッシュは"
        "「七月辛丑，僧辩纳贞阳侯萧明，自采石济江。」から始まる地の文。字を掲げるのは"
        "南史 梁宗室上（daizhigev20/史藏/正史/南史.txt L4506）"
        "「朗弟明字靖通，少被武帝亲爱，封贞阳侯。」。**南史は唐の避諱で「淵」を落として"
        "「明」と書く**ので鍵は諱の下1字になり、同定は同じ条の「封贞阳侯」による。"
        "**この1件だけ巻番号を書いていない** — `book_volumes.daizhige_spans` が0巻を返し、"
        "china-history 側も南史は卷一〜十（本紀）しか無いので、手で書かずに file と行で押さえた"
        "（2026-08-16・字ブロック4）。"
        "**2026-08-17 訂正: 「本文の巻見出しが卷二十二で途切れている」は事実に反する** — "
        "`南史卷N` の標識は**卷二から卷八十まで79件、欠落なく在る**。ただし行頭ではなく"
        "**前巻末尾の行末**に埋まる組版なので、行全体一致を要求する `_named_head_spans` が"
        "拾えず0巻になる。蕭淵明の L4506 は L4462 の `南史卷五十一` 配下＝**卷51（梁宗室上）**で、"
        "機械でも引ける。**同じ dict の `liang-yuzhangwang` は同じファイルから卷53 を引いており、"
        "2つの理由文が同一ファイルについて矛盾していた**（`book_volumes.py` に行末一致を"
        "足せば手書きは要らなくなる。2026-08-17 のブロック外の検証段）",
    "han-gaozu":
        "**本人のキャッシュ（漢書 高帝紀）は字を落とす** — 漢書の冒頭は"
        "「高祖，沛丰邑中阳里人也，姓刘氏。」で、同じ文を先に書いた史記が持つ"
        "「字季」をそこで切る。字を掲げるのは史記 卷八 高祖本紀第八"
        "（daizhigev20/史藏/正史/史记.txt L714）「高祖，沛丰邑中阳里人，姓刘氏，字季。」。"
        "**巻は手で書かず目次行と本文見出しの対から引いた**（目次 L10「卷八 高祖本纪第八」・"
        "本文見出し L713）。**キャッシュに史記の高祖本紀を足せば免除は消せる**"
        "（2026-08-16・字ブロック2）",
    "han-xuandi":
        "**本人のキャッシュ（漢書 宣帝紀・無注本145行）は「字」が0回** — 字を掲げるのは"
        "同じ漢書でも**顏師古注本**の側で、daizhigev20/史藏/正史/前汉书.txt L1050"
        "「孝宣皇帝【荀悦曰讳询字次卿訽之字曰谋…】」。**決め手は同じ注釈者の書き分け** — "
        "荀悦は「讳X字Y」（実在の字）と「X之字曰Y」（避諱代字）を使い分け、前者が付くのは"
        "高祖（L635）・成帝（L1151）・宣帝の3人だけで、他の7帝は後者しか持たない"
        "（武帝 L934「讳彻之字曰通」）。**この書は本文に巻の見出しを持たず "
        "`book_volumes.daizhige_spans` が0巻を返す**ので、南史（liang-xiaoyuanming）と同じく"
        "**手で巻を書かずファイルと行で押さえた**。外部の証人は6書で一致"
        "（藝文類聚 L2278・資治通鑑注 L2354・文選李善注 L327・太平御覧 L9651・"
        "冊府元亀 L282・名疑 L311）。**キャッシュを顏師古注本の宣帝紀へ広げれば免除は消せる**"
        "（2026-08-16 の字の検証段〔facts〕が見つけた）",
    "shiguo-nantang-lijing":
        "**本人のキャッシュ（新五代史 南唐世家）は字を置かない** — 字を掲げるのは十国春秋"
        "（daizhigev20/史藏/载记/十国春秋.txt L1413）「元宗名璟字伯玉烈祖长子母元敬皇后"
        "初名景通」。**十国春秋は「〈廟号〉名〈諱〉字〈字〉」を書式として持ち、同じ政権の"
        "李昪「字正伦」（L1398）の出どころでもある**（前蜀 王建・王衍、後蜀 孟知祥も同型）。"
        "**この書も本文に巻の見出しが無く `daizhige_spans` が0巻を返す**ので、手で巻を"
        "書かずファイルと行で押さえた。**キャッシュに十国春秋の南唐 元宗の条を足せば"
        "免除は消せる**（2026-08-16 の字の検証段〔facts〕が見つけた）",
    "tang-gaozu":
        "**本人のキャッシュ（舊唐書 高祖本紀）の外に名乗りが在る** — 舊唐書の冒頭は"
        "「姓李氏，讳渊。」で閉じて字を挟まない。字を掲げるのは新唐書 卷一 本紀第一"
        "（daizhigev20/史藏/正史/新唐书.txt L256）「高祖神尧大圣大光孝皇帝讳渊，字叔德，"
        "姓李氏，陇西成纪人也。」。**巻は手で書かず目次行と本文見出しの対から引いた**"
        "（目次 L3「卷一 本纪第一」・本文見出し L255「本纪第一　高祖」）。"
        "**唐は本紀のキャッシュが舊唐書なので、新唐書に字を持つ帝はこの免除に入る**"
        "（2026-08-16・字ブロック2）",
    "tang-gaozong":
        "**本人のキャッシュ（舊唐書 高宗本紀）の外に名乗りが在る** — 舊唐書は諱の次に"
        "続柄が来て字が入らない。字を掲げるのは新唐書 卷三 本紀第三"
        "（daizhigev20/史藏/正史/新唐书.txt L317）「高宗天皇大圣大弘孝皇帝讳治，字为善，"
        "太宗第九子也。」。**巻は手で書かず目次行と本文見出しの対から引いた**"
        "（目次 L5「卷三 本纪第三」・本文見出し L316「本纪第三　高宗」）"
        "（2026-08-16・字ブロック2）",
    "beiwei-taiwudi":
        "**本人のキャッシュ（魏書 卷四 世祖紀）の外に名乗りが在る** — 字を掲げるのは"
        "泰常七年四月甲戌に泰平王へ封じた条で、魏書 卷三 帝紀第三 太宗紀"
        "（daizhigev20/史藏/正史/魏书.txt L379）「封皇子焘为泰平王，焘，字佛厘」。"
        "**巻は手で書かず目次行と本文見出しの対から引いた**（目次 L7「卷三 帝纪第三」・"
        "本文見出し L338「帝纪第三 太宗纪」・次の見出しが L388「帝纪第四 世祖纪上」）。"
        "南朝側の2書も同じ語で呼ぶ（音写は佛狸） — 宋書 卷九十五 列傳第五十五 索虜"
        "（宋书.txt L7063）「子焘字佛狸代立」・南齊書 卷五十七 列傳第三十八 魏虜"
        "（南齐书.txt L3460）「子寿，字佛狸，代立」。**南齊書は諱を「寿」と書く**ので"
        "字面では本人に結びつかない — 同定は同じ条の「年号太平真君」と、"
        "明元帝（木末）→ この人 → 晃子 浚（烏雷直勤）という継承の並びによる。"
        "**キャッシュを魏書 卷三まで広げれば免除は消せる**（2026-08-16・字ブロック1）",
    "beiwei-xiandi":
        "**魏書・北史はこの人物の字を書かない** — 献文帝紀の冒頭は「显祖献文皇帝，讳弘，"
        "高宗之长子」で字を挟まない。字を掲げるのは南朝の側で、南齊書 卷五十七 列傳第三十八 "
        "魏虜（daizhigev20/史藏/正史/南齐书.txt L3460）「子弘，字万民，立，年号天安」。"
        "**同じ人物に宋書は別の字を当てる** — 卷九十五 列傳第五十五 索虜（宋书.txt L7125）"
        "「子弘之字第豆胤代立」で、こちらは鮮卑名の音写。**採ったのは万民の側だけ**で、"
        "理由は 2026-08-16 のユーザー決定の枠（北朝の書が字と書けば採る／南朝の書だけの"
        "ときは音写・称号を採らず漢式の字として立つ形だけ採る）による。第豆胤は"
        "断片の `discrepancies` に残した（2026-08-16・字ブロック9）",
    "han-chengdi":
        "**本人のキャッシュ（漢書 卷十 成帝紀第十）は「宣帝爱之，字曰太孙」と"
        "動詞をはさむ書き方**なので「字〈値〉」の隣接にならない。隣接した形で書くのは"
        "元后傳の側で、漢書 卷九十八（daizhigev20/史藏/正史/汉书.txt L4305・"
        "本文見出し L4303「卷九十八」）「宣帝爱之，自名曰骜，字太孙，常置左右」。"
        "**採った理由は隣接ではない** — 元后傳も「〈人〉爱之，…字〈X〉」という同じ枠で、"
        "動詞用法として退けた西夏書事「德明爱之，字为嵬埋」と形が並ぶ。**決め手は"
        "名の行き先が他にあるかどうか**で、嵬理は宋史が「小字」と呼ぶので幼名の欄が"
        "持つのに対し、**太孫を小字と呼ぶ書は無い**（漢書1冊に「小字」の語が0件）。"
        "**キャッシュに卷九十八を足せば免除は消せる**（2026-08-16・字ブロック1）",
    "beiwei-yuanye":
    ...
    "wei-mingdi":
        ...
        "符仲はこちらのコーパス側の誤植。"
        "**2026-08-17 訂正: コーパスの入れ替えは要らない** — 同じコーパスの"
        "`daizhigev20/史藏/正史/三国志.txt` L444 が既に「明皇帝讳叡字元仲」を持ち、"
        "china-history 自身の 明帝纪-译文.html／段译.html も同じ箇所を元仲と出す。"
        "**照合先を1冊足すか、この章だけキャッシュの生成元を daizhige に振れば今日消せる**。"
        "なお旧文の「同じ字は他に1件も無い」は字としては正しいが、文字列 `符仲` は"
        "`china-history/后汉书/列传/第四十二章-王充王符仲长统列传` にも出る（篇題の部分列）"
        "（2026-08-17 のブロック外の検証段）",
    "xiyan-murongyong":
...
CHILDHOOD_ALLOW = {
    ...
    "qi-gaodi":
        "**本人の底本（南齊書）が簡体の統合先字で写っているため、保存値の字体と隣接しない**。"
        "南齊書 卷1（南齐书.txt L66）は「讳道成，字绍伯，姓萧氏，小讳斗将」と『斗』で書くが、"
        "**繁体を保つ本文はすべて鬬／鬭**で、保存値「闘将」はその新字体。"
        "証人は5書 — 通志（史藏/别史/通志.txt L2352）「小字鬬将」・資治通鑑注（史藏/编年/"
        "资治通鉴注.txt L14072）「字绍伯，小字鬬将」・小字錄（子藏/类书/小字录.txt L55・"
        "見出し語も L54「鬬将」）・萬姓統譜（子藏/类书/万姓统谱.txt L1222）・"
        "説郛（子藏/笔记/说郛.txt L36183）は鬭。**簡体では斗が鬥/鬬/鬭 の統合先**なので、"
        "南齊書・南史・冊府元龜のような簡体化された電子テキストは字体の証人にならない。"
        "`norm_for_match` が同一視するのは鬥/斗だけで鬬・鬭・闘は別クラスなので、"
        "この免除を外すには繁体を保つ底本をキャッシュに足すしかない（2026-08-17 の"
        "ブロック外の検証段〔facts〕が見つけた）",
    "shiguo-nantang-libian":
        "**本人のキャッシュ（新五代史 南唐世家）は小字を落とす**。値を書くのは十国春秋"
        "（史藏/载记/十国春秋.txt L1398）「烈祖姓李名昪字正伦小字彭奴徐州人也」で、"
        "南唐書3種も同じ1行に字と小字を並べる — 南唐書（史藏/载记/南唐书.txt L27）"
        "「名昪，字正伦。小字彭奴。」・陸氏南唐書（同 陆氏南唐书.txt L69）・"
        "馬氏南唐書（同 马氏南唐书.txt L24）「先主姓李唐宗室裔也小字彭奴」。"
        "**十国春秋は本文に巻の見出しを持たず `daizhige_spans` が0巻を返す**ので、"
        "南史・漢書顏師古注本と同じく手で巻を書かずファイルと行で押さえた"
        "（2026-08-17 のブロック外の検証段〔facts〕が見つけた）",
    "tang-gaozong":
        "**語順が逆で、しかも本人の本紀ではなく他者の列傳に在る**。キャッシュは舊唐書の"
        "高宗紀だが、値を書くのは新唐書 卷80 列傳第五 太宗子（濮恭王泰傳・新唐书.txt L10361）"
        "の割注「公劝我立雉奴，雉奴仁懦，得无为宗社忧，奈何？」に続く「雉奴，高宗小字」で、"
        "**人物を諱ではなく廟号で名指す**（後唐明宗「邈佶烈，蓋嗣源小字也」と同じ型）。"
        "外部対照は小字錄（宋 陳思・四庫本・子藏/类书/小字录.txt L96）"
        "「唐髙宗讳治字为善小字雉奴太宗第九子」。舊唐書に「雉奴」は0件。"
        "**3つ目の証人は資治通鑑の胡三省注**（daizhigev20/史藏/编年/资治通鉴注.txt L21332）"
        "「字为善，小字雉奴，太宗第九子也」で、字と小字を同じ順に並べる"
        "（同じ日の字の検証段が見つけた）。"
        "2026-08-16 の検証段（幼名 facts）が見つけた",
    # 燕の4人は**晉書 載記が小字を書かない**（載記は「〈姓名〉，字〈字〉，〈続柄〉」で閉じる）。
    # 書くのは十六国春秋の側で、書き出しが「〈姓名〉字〈字〉小字〈小字〉」の1行に3つを並べる
    # （2026-08-16・幼名ブロック2）
    "qianyan-murongjun":
        "キャッシュは晉書 卷一百十 慕容儁載記で、載記は小字を書かない（冒頭は"
        "「慕容俊，字宣英，皝之第二子也」）。小字を掲げるのは十六国春秋 卷二十五"
        "（十六国春秋.txt L408）「慕容儁字宣英小字贺赖跋」。"
        "**底本は諱を「俊」、十六国春秋は「儁」と書く**ので、同定は字「宣英」と続柄「皝之第二子」による",
    "houyan-murongchui":
        "キャッシュは晉書 卷一百二十三 慕容垂載記で、載記は小字を書かない。"
        "小字を掲げるのは十六国春秋 卷四十一（十六国春秋.txt L753）"
        "「慕容垂字叔仁[一作道明]小字阿六敦」。**晉書は字を「道明」と書き、"
        "十六国春秋は「叔仁」を本文・「道明」を割注に置く**ので、同定は続柄「皝第五子」による",
    "houyan-murongbao":
        "キャッシュは晉書 卷一百二十四 慕容宝載記で、載記は小字を書かない。"
        "小字を掲げるのは十六国春秋 卷四十六（十六国春秋.txt L778）「慕容宝字道佑小字库勾」"
        "（晉書の字「道祐」と同じ音の異表記）",
    "houyan-murongxi":
        "キャッシュは晉書 卷一百二十四 慕容熙載記で、載記は小字を書かない。"
        "小字を掲げるのは十六国春秋 卷四十八（十六国春秋.txt L793）「慕容熙字道文小字长生」",
    # 五代十国の3人は**値の在り処が本人の紀・世家の外**（2026-08-16・幼名ブロック3）
    "wudai-houtang-zhuangzong":
        "キャッシュは舊五代史の荘宗紀で、**舊五代史は荘宗に小字を書かない**。"
        "掲げるのは新五代史 卷37 伶官傳（新五代史.txt L933）「其小字亚子，当时人或谓之亚次」で、"
        "俳優・音曲を好んだ帝の性向を述べる書き出しに置かれる。"
        "舊五代史がこの書式を持たないわけではなく、閔帝には帝紀の冒頭1行に"
        "「闵帝，讳从厚，小字菩萨奴」を置く",
    "shiguo-nanhan-liubin":
        "**値は本人の世家に在るが語順が逆**。新五代史 卷65 南漢世家（新五代史.txt L2229）で"
        "高祖 劉龑が臨終に「呼洪度、洪熙小字曰：『寿、俊虽长，然皆不足任吾事』」と2子を呼ぶ形で、"
        "「小字」が値の**前**に来ないので隣接のゲートには掛からない"
        "（後唐明宗の「邈佶烈，盖嗣源小字也」と同じ型）。洪度＝のちの玢",
    "shiguo-nanhan-liusheng":
        "同じ条（新五代史.txt L2229）の「洪熙」＝のちの晟。"
        "**本人の世家のキャッシュは弑逆のあとから始まる**ので、この条は範囲の外に在る",
    "beiqi-feidi-gaoyin":
        "**名乗る底本の北齊書は小名を書かない**。北齊書 卷五の冒頭は「废帝，讳殷，字正道，"
        "文宣皇帝之长子也」で、同じ位置に小名を挟むのは北史 卷七（北史.txt L878）"
        "「废帝殷，字正道，小名道人，文宣帝之长子也」。**2026-08-15 の廟号ブロック7は"
        "「北齊書には無い」として欄へ入れなかった**が、その後 2026-08-16 に"
        "「正史の外の書も名前欄の証人にしてよい」と決まった枠（Issue #126 の決着済み）に沿って"
        "北史を証人に採った（2026-08-16・幼名ブロック1の後追い）。"
        "**2026-08-17 訂正: 冒頭の「北齊書は小名を書かない」は事実に反する** — "
        "北齊書は帝紀の書き出しに置かないだけで、本文では2箇所書いている: "
        "卷十四 補列傳第六 高元海の条（北齐书.txt L622）「道人，济南王小名」と"
        "楊愔伝（同 L1870）「『道人』谓废帝小名」。免除そのものは"
        "**キャッシュ（帝紀のみ）の外に在る**ので妥当（2026-08-17 のブロック外の検証段）",
    "qianliang-zhangzuo":
        "キャッシュは晉書 載記の張祚の条で、載記は小字を書かない。**2つの書が独立して書く** — "
        "十六国春秋 卷七十三（十六国春秋.txt L1286）「张祚字太伯小字螽斯」と、"
        "魏書 卷九十九（魏书.txt L6300）が張重華末年の凶兆の条で「螽斯是祚小字」と"
        "**名指しで説明する**（門に集まった虫の螽斯を都尉常據が張祚の小字に掛けて諫めた条）"
        "（2026-08-16・幼名ブロック1の後追い）",
    "xiyan-murongchong":
        "キャッシュは資治通鑑から切った4行で、慕容沖には載記も本紀も立たない。"
        "小字を書くのは晉書 卷一百十四 苻堅載記下（晋书.txt L8221）"
        "「冲小字凤皇，至是，终为坚贼，入止阿房城焉」で、**苻堅が鳳凰を待って桐竹を植えた話の"
        "落ちとして書かれる**。**2026-08-16 の字ブロック7は「鳳皇はコーパスのどの書にも無い」と"
        "書いたが誤りで、同じ日の後追いで晉書の側に在ることが分かった**"
        "（字の欄が判別不能なのは変わらない — これは小字であって字ではない）",
    "beiwei-yuanye":
        "魏書はこの人物に帝紀を立てず、キャッシュは擁立・廃位の条だけを切っている。"
        "小字を掲げるのは列傳の側で、魏書 卷十九下 景穆十二王下（魏书.txt L1433）"
        "「晔字华兴，小字盆子」・北史 卷十八（北史.txt L2319）が同文。"
        "さらに北史 卷四十八 爾朱世隆伝（北史.txt L7944）が擁立の条で「晔小名盆子」と繰り返し、"
        "**帝としての彼を指す文脈**（「与度律等共推长广王晔为主」の直後）で同定が付く"
        "（2026-08-16・幼名ブロック1）",
    "beiqi-wuchengdi":
        "帝紀の書き出しには無い。北齊書（北齐书.txt L622）の童謡の注"
        "「盖指武成小字步落稽也」と高浚伝「呼长广小字曰：『步落稽，皇天见汝！』」。"
        "**2026-08-17 訂正: L622 は独孤永業伝ではない** — 卷十四 補列傳第六"
        "（上洛王思宗の子・高元海の条）で、独孤永業伝は同書 L2391。"
        "**あわせて注意**: daizhige の北齊書は卷五・六・八・十・十四・三十四が見出しに"
        "「补」を持ち、本文が北史から補われている。**これらの巻は北史とは別の証人に数えない**"
        "（2026-08-17 のブロック外の検証段）",
    "houzhao-shile":
        "キャッシュは晉書 載記で、載記は小字を書かない。魏書（魏书.txt L5978）"
        "「羯胡石勒，字世龙，小字匐勒」。"
        "**2026-08-17 追記: 名乗りの枠が書で割れている** — 晉書 載記（晋书.txt L7670）は"
        "同じ別名を**「初名」**として書き（字面も PUA 1字で「匐勒」ではない）、"
        "十六国春秋（十六国春秋.txt L158）は「石勒字世龙初名背[魏书作小字匐勒]」と"
        "**校勘注で両者を等置**する。枠の数は初名2書 対 小字1書。"
        "**魏書が literal に「小字」と書く**ので欄の定義は満たすが、"
        "同人物の断片台帳 `discrepancies` は「初名は幼名でも字でもない別名なので欄を立てない」と"
        "宣言しており、配布物の値と方針が食い違ったままである"
        "（2026-08-17 のブロック外の検証段。値の当否はここでは決めていない）",
    "liang-houjing":
        ...
        "**2026-08-17 追記: 同じ詩句を最古の梁書は小字と読まない** — 梁書（梁书.txt L1793）は"
        "「掘尾狗子、山家小儿，皆猴状」と並べ、南史 L6601 が同じ並びを"
        "「狗子，景小字，山家小儿，猴状」と**書き分けて改めた形**。梁書（636年）と"
        "隋書五行志・南史（唐初）で読みが割れている。**3件目に見える文献通考**"
        "（史藏/政书/文献通考.txt L31349）「侯景小字狗子。」は**隋書五行志の丸写し**で"
        "独立の証人ではない（2026-08-17 のブロック外の検証段）",
    "wudai-houliang-zhuyougui":
        "舊五代史（旧五代史.txt L388）の列伝「友珪，小字遥喜」"
        "（同じ条に命名の由来「故字之曰遥喜」も在る）。"
        "**2026-08-17 訂正2点** — (1) 「キャッシュは舊五代史の帝紀側」は誤りで、"
        "友珪に帝紀は無く `_corpus_cache` の中身は太祖紀末・末帝紀からの地の文。"
        "(2) 典拠が**本文欠落の既知な1冊版だけ**になっていた（`旧五代史.txt` は全1,606行で、"
        "目次に在る巻の本文が落ちている）。**四庫版にも同文が在る** — "
        "`daizhigev20/史藏/正史/旧五代史四库.txt` L1118「友珪小字遥喜」。"
        "新五代史は「遥喜」0件（2026-08-17 のブロック外の検証段）",
    "xixia-jingzong":
    ...
    "wudai-houtang-mingzong":
        ...
        "**2026-08-17 追記: 3つ目の在り処で枠が割れる** — 新五代史 明宗紀の冒頭"
        "（新五代史.txt L145）「父霓，为雁门部将，生子邈佶烈 ，以骑射事太祖」は"
        "同じ語を**生まれたときの名**として書き（続けて「太祖养以为子，赐名嗣源」）、"
        "舊五代史・新五代史 王彦章伝の「小字」と割れている。値は動かさないが、"
        "**免除の理由文が小字側だけを引いていた**（2026-08-17 のブロック外の検証段）。"
        "典拠の舊五代史は四庫版 L1498 にも同文",
}
...
POSTHUMOUS_STAGES_ALLOW = {
    ...
    # 2026-08-16 のユーザー決定でまとめて塞いだ5件（それ以前から赤かったぶん）。
    # **3件は同じ書の別巻・2件は別の書**で、どちらも証人は断片が持っている。
    ("hou-han-shaodi-bian", "懐王"):
        "**同じ書の別巻** — 弘農王への諡の条は後漢書 皇后紀下（china-history "
        "`后汉书/本纪/第十二章-皇后纪下-原文.html` L58「初平元年二月，葬弘农王于故中常侍赵忠成圹中，"
        "谥曰怀王」）に在り、少帝弁のキャッシュ（本紀）へ入っていない",
    ("houliang-houzhu", "孝靖帝"):
        "**別の書** — 西梁 後主 蕭琮に諡を贈ったのは唐初に梁を僭称した蕭銑で、条は"
        "舊唐書 L9829「伪谥其从父琮为孝靖帝」・新唐書 L10943「追谥从父琮为孝靖帝」に在る。"
        "琮の名乗る周書・隋書・北史はいずれも「赠左光禄大夫」で終わり諡を書かない",
    ("liang-xiaoyuanming", "閔皇帝"):
        "**別の書** — 梁書には諡の条が無く（本文は「贞阳侯」「建安公渊明」と封爵で呼ぶ）、"
        "南史 卷五一 L4511「追諡明曰闵皇帝」と北齊書 卷三三 L1774「追谥明曰闵皇帝」が持つ。"
        "贈ったのは王琳が擁立した永嘉王 蕭莊の政権",
    ("beiwei-tuobayu", "隠"):
        "**同じ書の別巻** — 南安王余の諡の条は魏書 卷十八 列傳 L1262「高宗葬以王礼，谥曰隐」に在り、"
        "本人のキャッシュ（世祖紀下の末尾）へ入っていない（2026-08-14 にこの条から転記した）",
    ("tang-wuzetian", "則天順聖皇后"):
        "**同じ書の別巻** — 天宝八載の加諡の条は舊唐書 卷九（玄宗紀下）L665"
        "「太穆、文德、则天、和思、昭皇后，皆加“顺圣皇后”之字」に在り、"
        "本人のキャッシュ（卷六 則天皇后紀）へ入っていない。崩御条の初諡"
        "「則天大聖皇后」はキャッシュ内（L37）に在る",
}
