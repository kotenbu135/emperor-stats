...
def bucket_of(kind, title):
    ...
    if kind.endswith("载记"):
        return "載記"
    # **世家は ANNALS に入れない。** 新五代史では十国の君主が名乗る底本そのもの
    # （載記と同じ位置）だが、史記の世家は諸侯なので同じ扱いにすると候補が消える。
    # 判定はこの道具の仕事ではないので、候補として出して人に読ませる側へ倒す
    # （2026-08-13 に足した — それまで「吴世家第一」が見出しに当たらず、
    # 新五代史は24件の目次見出しだけを拾って「帝紀の外の候補0件」を出していた）
    if kind.endswith("世家"):
        return "世家"
    if kind.endswith("志"):
