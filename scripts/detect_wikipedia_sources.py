...
HISTORY_KEYWORDS = [
    ...
    # 元号の考証書（daizhige 政书。ALLOWLIST の元末の条で既に許している書を書名として通す）
    "歴代建元考", "历代建元考",
    "華陽國志", "華陽国志", "华阳国志",
...
ALLOWLIST_PAGES = {
    ...
    "近現代の学術的に信頼できる複数情報源",
    # 南康公主=東晋明帝女の典拠（kinship フェーズ3ブロック1）。晋書は「选尚南康长公主」とのみ記し
    # 父帝を明記しないため、東晋の史書『晋中興書』の佚文（太平御覧巻一百五十二所引・
    # daizhigev20/子藏/类书/太平御览.txt 行11741 で逐語実在確認済み）を典拠とする。
    # 世説新語劉孝標注引続晋陽秋・建康実録巻九も一致（2026-07-24 確認）
    "太平御覧 巻一百五十二 皇親部（晋中興書佚文）",
}
