...
POSTHUMOUS_STAGE_FULL_MISMATCH = {
    ...
    # 清史稿は**加上の条が形を書かない**（「加上〈廟号〉尊谥」で止める）ので、形を書く条が
    # 在る回だけが段になる。冒頭形はその列の外の回を指すことがあり、**長さの向きで型が割れる**:
    # 冒頭形のほうが**長い**＝後の加上の結果（高宗・宣宗）／**短い**＝形を書かない初諡の側
    # （聖祖）。どちらも清史稿の中に形を授ける条が無いので段として立てない。
    # **どちら向きでも「冒頭形＝earlier」と決めつけない** — 向きを取り違えると免除の理由が
    # 逆になり、次に読む人が段の順序を反対に読む
    "qing-shengzu": "冒頭形は「中和」の2字を**欠く**（段より短い）。形を書く条は乾隆元年の"
                    "加上だけで、その前の雍正元年の初諡を清史稿は形なしで書くため、"
                    "冒頭形が指す回を段に立てられない",
    "qing-gaozong": "冒頭形は「欽明」の2字を**持つ**（段より長い）。嘉慶四年の条はこれを持たず、"
                    "宣宗紀の加上の条は形を書かない",
    "qing-xuanzong": "冒頭形は「寛定」の2字を**持つ**（段より長い）。道光三十年の条はこれを持たず、"
                     "文宗紀の加上の条は形を書かない",
    "qing-wenzong": "冒頭形は「端仁」だが咸豐十一年の条は「謙仁」で、端／謙の1字が"
                    "清史稿の中で食い違う（徳宗紀の加上の条は形を書かない）",
    # 遼史・金史で出た2型。**どちらも「長い／短い」だけでは足りない**:
    # 道宗は書が冒頭にだけ短縮形を掲げる（授与そのものは1回しか無い）、
    # 金太祖は増字と**字順の入れ替え**が同時に起きる（長短の比較では型が決まらない）
    # 2026-08-13 に2件（皇帝の諡でない諡・偽諡をこの欄へ入れる決定で出た型）
    "liu-song-shundi": "冒頭形は「順皇帝」だが授与条は「谥曰顺帝」と**略した形**"
                       "（宋書の中の書き分け）。段には条が書く形を採る",
    # 2026-08-15 のユーザー決定（輯本・私撰と編年・注を名前欄の witness に採る）で出た型。
    # **これまでの免除はすべて「同じ書の中の食い違い」だったが、この3件は書がまたがる** —
    # 正史が〈諡字〉帝／1字で書き、〈X〉皇帝の形は輯本・編年の側にしかない。
    # 授与の回そのものは1回なので、長い形を段の2段目に立てない（立てると回数を偽る）
    "chenghan-lixiong": "段は晉書の「伪谥武帝，庙曰太宗」（〈諡字〉帝）だが、全長形は"
                        "十六国春秋別伝 L151「谥武皇帝，庙号太宗」・資治通鑑注 L9454。"
                        "同じ1回の授与を書が違う長さで書いている",
    "chenghan-lishou": "段は晉書の「伪谥昭文帝，庙曰中宗」だが、全長形は十六国春秋別伝 L155"
                       "「谥昭文皇帝，庙号中宗」。十六国春秋 L1459 は「谥曰昭文」と2字で書く",
    "shiguo-wu-yangpu": "段は新五代史 呉世家の「谥曰睿」（1字）だが、全長形は"
                        "十国春秋 L1025「追谥曰睿皇帝」・資治通鑑 L28882。同じ1回の授与",
    "sui-yangdi": "冒頭形「煬皇帝」は唐が贈った諡だが、**隋書はこの諡を授ける条を持たない**"
                  "（他書の諡なので当然）。段に立つのは皇泰主 侗が贈った「明」の1件だけで、"
                  "冒頭形とは別の授与を指す。**冒頭形のほうが後**（大業十四年の明→武徳中の煬）",
    "liao-daozong": "冒頭形「孝文皇帝」は乾統元年の条が授ける「仁聖大孝文皇帝」の短縮形で、"
                    "遼史は道宗だけ冒頭に短縮形を掲げる（加諡は1回も無い＝段は1つ）",
    "jin-taizu": "冒頭形は「…昭徳定功**仁明荘孝**大聖武元皇帝」だが皇統五年の加諡条は"
...
#
# **2026-08-13 のユーザー決定で枠が広がった** — 皇帝の諡でない諡（王諡・公諡・皇后の諡）も、
# 敵対する勢力が贈った諡（書が「伪谥」と呼ぶもの）も欄へ入れる。**形の資格では落とさず、
# その書がその語を諡と呼ぶかで決める**。ここに並ぶのは「皇帝」で結ばない4型:
#   (1) 王・公諡      … 「恭王」「閔悼王」「幽公」
#   (2) 廟号型2字     … 「哀宗」「昭宗」（金史は哀宗に掛かる語をすべて「谥」で書く）
#   (3) 「〈諡〉帝」   … 晉書 載記の「伪谥武帝」「伪谥昭文帝」・宋書の「谥曰顺帝」
#   (4) 諡字1字       … 「炀」「闵」「绍」（条が諡字だけを書く形）
# **形は条が書いている形をそのまま採る**（ゲートFが本人のキャッシュに連続一致を見るため、
# 「明」を「明皇帝」へ伸ばすと空振りする）。
POSTHUMOUS_STAGE_NON_IMPERIAL = {
    ("ming-daizong", "戾"): "郕王へ落として与えた王諡なので皇帝号で結ばない"
                            "（成化十一年に帝号を復し「恭仁康定景皇帝」を追諡）",
    ("chen-houzhu", "煬"): "隋 仁寿四年に薨じたあと追贈された諡で、**隋が長城県公として"
                           "贈った1字**（陳書「追赠大将军，封长城县公，谥曰炀」）。"
                           "皇帝としての諡ではないが書が「谥」と呼ぶので段に置く",
    ("houliang-houzhu", "孝靖帝"): "隋に併合されて莒国公となったのち、從子の蕭銑が"
                                   "贈った諡（舊唐書 蕭銑伝「伪谥其从父琮为孝靖帝」）。"
                                   "「〈諡〉帝」の形で結び「皇帝」で終わらない。"
                                   "**2026-08-14 に周書1冊で閉じた read-absent を訂正して足した**"
                                   "（諡を贈った主体が別政権なので、その政権の書には条が無い）",
    # (1) 王・公諡
    ("hou-han-shaodi-bian", "懐王"): "廃位して弘農王へ落とされ、董卓に鴆殺されたあとの王諡"
                                      "（後漢書 皇后紀下「初平元年二月，葬弘农王于故中常侍"
                                      "赵忠成圹中，谥曰怀王」）。**この条は本紀ではなく皇后紀に在る**"
                                      "（唐姫伝の直前）ので、帝紀だけを読むと取りこぼす"
                                      "（2026-08-15・廟号ブロック8）",
    ("qi-hailingwang", "恭王"): "廃位して海陵王へ落としたうえでの王諡"
                                "（南齊書「依东海王故事。谥曰恭王」）。冒頭形も"
                                "「海陵恭王昭文」とこの王諡を冠して立つ",
    ("beiqi-feidi-gaoyin", "閔悼王"): "廃位して済南王へ落としたうえでの王諡"
                                      "（北齊書「大宁二年，葬于武宁之西北，谥闵悼王」）",
    ("qianqin-fusheng", "厲王"): "**廃して越王へ落としたうえで殺したあとの王諡**"
                                  "（晉書 載記第十二「引生置于别室，废之为越王，俄而杀之…"
                                  "时年二十三，在位二年，伪谥厉王」＝晋书.txt L8142）。"
                                  "同じ載記の他の君主は「伪谥〈X〉皇帝，庙号〈Y〉」で結ぶので、"
                                  "**この1人だけ皇帝号で結ばないのは廃位が理由**"
                                  "（2026-08-15・廟号ブロック4）",
    ("chenghan-liqi", "幽公"): "廃されて邛都県公へ落としたのちの公諡"
                               "（晉書 載記「谥曰幽公」）。公諡は皇帝号で結ばない",
    ("beiwei-tuobayu", "隠"): "宗愛に殺されたのち、文成帝が**王礼で葬った際に贈った諡**1字"
                              "（魏書 卷十八 列傳第六「高宗葬以王礼，谥曰隐」）。"
                              "**2026-08-14 に本人のキャッシュ（世祖紀下の末尾）だけで閉じた"
                              "read-absent を訂正して足した** — 諡の条は帝紀ではなく列傳に在り、"
                              "短縮諡80人を読み切った側にも同じ型の取りこぼしが在ったことになる",
    ("jin-hailingwang", "煬"): "大定二年に廃位して海陵郡王へ落としたうえでの**王としての諡**1字"
                               "（金史「大定二年，降封为海陵郡王，谥曰炀」）。"
                               "**2026-08-13 の決定を受けた転記でこの1人だけ落としていた**"
                               "（残量表は衛紹王と2件一緒に決めると書いていたのに、"
                               "衛紹王だけ入れて閉じた）。同日中に足した",
    ("jin-weishaowang", "紹"): "貞祐四年に旧封の衛王を復したうえでの**王としての諡**1字"
                               "（金史「贞祐四年，诏追复卫王谥曰绍」）。"
                               "「衛紹王」の紹はこの諡",
    # (2) 廟号型2字（金史は哀宗に掛かる語をすべて「谥」で書き、「庙号」と書き分けない）
    ("jin-aizong", "哀宗"): "蔡州陥落の日に末帝が贈った諡。**金史はこれを「谥」と書く**"
                            "（「末帝退保子城，闻帝崩，率群臣入哭，谥曰哀宗」）ので、"
                            "形が廟号型2字でも諡の欄で受ける（2026-08-11 の決定の延長）",
    ("jin-aizong", "昭宗"): "宋へ送款した際に諸帥が改めて贈った第2段"
                            "（金史「遂发丧设祭，谥哀宗曰昭宗」）。同じく書が「谥」と呼ぶ",
    # **閩の2人（康宗・景宗）は 2026-08-15 に外した。** 新五代史だけを見て「廟号型2字を
    # 谥と書く書」と読んでいたが、資治通鑑 L28915 と十国春秋 L3885/L3906 が同じ条で
    # **康宗・景宗を「庙号」と書き、諡は別に長い形を書く**（「谥闽主曰圣神英睿文明广武
    # 应道大弘孝皇帝，庙号康宗」）。新五代史自身も父 王鏻には「谥曰惠皇帝，庙号太宗」と
    # 書き分けている。2書対1書で廟号と読み、康宗・景宗は templeName へ移した。
    # **金 哀宗はこの型ではない** — 金史は哀宗に掛かる語をすべて「谥」で書き、別書に
    # 「庙号」と書く証人が無い（だから上の2件は残す）。**2026-08-15 に実測した**:
    # daizhigev20 の 正史・载记・编年・别史 255ファイルで「哀宗」「昭宗」が「庙号」と
    # 同じ窓（前後40字）に来る当たりは1件だけで、それは北元 昭宗（新元史 L4096
    # 「群臣上庙号曰昭宗皇帝」）＝別人。閩の2人は同じ走査で2書が「庙号」と書いていた
    # (3) 「〈諡〉帝」で結ぶ略形（冒頭の名乗りは全長形のことがある＝FULL_MISMATCH 側で受ける）
    ("liu-song-shundi", "順帝"): "宋書の授与条が「谥曰顺帝」と略した形"
                                 "（冒頭形は「順皇帝」で、条の側が略記）",
    ("chenghan-lixiong", "武帝"): "晉書 載記の死去条の要約「伪谥武帝，庙曰太宗」。"
                                  "**「伪」は書き手の立場を表す語**で、諡が贈られなかった"
                                  "ことを意味しない（2026-08-13 のユーザー決定）",
    ("chenghan-lishou", "昭文帝"): "同じく晉書 載記の「伪谥昭文帝，庙曰中宗」",
    ("qi-hedi", "和帝"): "南齊書「戊辰，薨，年十五。追尊为齐和帝，葬恭安陵」。"
                          "**書は王朝名を冠して「齊和帝」と書く**（梁の側から書いている"
                          "ため。梁書 武帝紀も「追谥为齐和帝」）ので、諡は「和」＝「和帝」。"
                          "2026-08-13 に `read-absent` から移した"
                          "（「谥」の語彙だけで走査したための取りこぼし）",
    ("sui-yangdi", "明"): "皇泰主 侗が贈った諡「谥帝曰明，庙号世祖」（隋書 卷五十九・"
                          "越王侗伝）。**唐が贈った煬皇帝より先**に立つ段で、"
                          "条が諡字1字だけを書くので伸ばさない",
    # (4) 諡字1字
    ("wudai-houtang-mindi", "閔"): "後晋高祖が贈った諡1字（舊五代史"
                                   "「晋高祖即位，谥曰闵」）。冒頭形「闵帝」は"
                                   "書がこの諡から立てた名乗り",
    ("shiguo-wu-yangpu", "睿"): "禅位して譲皇となったあとに贈られた諡1字"
                                "（新五代史 呉世家「十二月，溥卒于丹阳，年三十八，"
                                "谥曰睿」）。**在位中の「加尊号睿聖文明孝皇帝」は"
                                "生前の尊号**で段ではない（同じ世家の中に両方が在る）",
    ("shiguo-nanhan-liubin", "殤"): "弑逆されたあと弟 晟が贈った諡1字"
                                     "（新五代史 南漢世家「玢立二年，年二十四，谥曰殇」）。"
                                     "**南漢は同じ世家の中で全長形（劉晟の"
                                     "「文武光聖明孝皇帝」）と1字を書き分ける**",
    # 皇后の諡（自身の遺制が帝号を去ることを命じている）
    ("tang-wuzetian", "則天大聖皇后"): "舊唐書の崩御条「谥曰则天大圣皇后」。"
                                        "**皇后としての諡**で、遺制が「令去帝号，"
                                        "称则天大圣皇后」と帝号を去ることを命じている。"
                                        "皇帝として収録している人物で、贈られた諡は"
                                        "いずれも皇后で結ぶ（2026-08-13 のユーザー決定）。"
                                        "**2026-08-15 に「この形しかない」を訂正した** — "
                                        "天寶八載の加諡がもう1段ある（下の行）",
    ("tang-wuzetian", "則天順聖皇后"): "天寶八載（749）の加諡。舊唐書 玄宗紀下"
                                        "「太穆、文德、则天、和思、昭皇后，皆加"
                                        "「顺圣皇后」之字」で、**結果の形は中宗紀・"
                                        "高宗諸子が「则天顺圣皇后」と逐語で掲げる**。"
                                        "初諡と同じく皇后で結ぶので同じ理由で免除する"
                                        "（2026-08-15・廟号ブロック10）",
}
# 充足のラチェット。転記は各ブロックの中で進むので強制はせず、**減ったら落ちる**
# （SCHEMA_CHANGE_CHECKLIST.md 手順5）。実測を書く
POSTHUMOUS_STAGES_FLOOR = 182
