...
POSTHUMOUS_STAGE_FULL_MISMATCH = {
    ...
    # 清史稿は**加上の条が形を書かない**（「加上〈廟号〉尊谥」で止める）。冒頭形は
    # その加上の結果を示すが、清史稿の中に形を授ける条が無いので段として立てられない。
    # 宋の1字違いとは型が別で、**冒頭形のほうが長い**のが目印
    "qing-shengzu": "冒頭形は「中和」の2字を持つが、形を書く条は乾隆元年の加上"
                    "（中和を含む形）だけで、雍正元年の初諡は清史稿が形を書かない",
    "qing-gaozong": "冒頭形は「欽明」の2字を持つが、嘉慶四年の条はこれを持たず、"
                    "宣宗紀の加上の条は形を書かない",
    "qing-xuanzong": "冒頭形は「寛定」の2字を持つが、道光三十年の条はこれを持たず、"
                     "文宗紀の加上の条は形を書かない",
