...
- **定型を採らない書き方**（「字曰〈…〉」「字為〈…〉」のような動詞用法）。この形は
  ...
  **ただし動詞用法だから字ではない、とは限らない**（2026-08-16 に訂正した）。
  前漢 成帝の「字曰太孫」はここに偽陽性として挙げていたが、**同じ漢書の元后傳が
  「自名曰骜，字太孙」と隣接した形でも書く**（卷九十八・L4305）。2箇所とも「字」と
  呼ぶので転記に改め、ゲートCは免除（`COURTESY_ALLOW`）で受けた。
  動詞用法に当たったら**同じ書の別の巻を見に行く**のが正しい読み方になる
