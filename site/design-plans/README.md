...
## 2026-07-31 の再構築（いま有効なもの）

| ファイル | 中身 |
|---|---|
| `STACK_OPTIONS_2026-07-31.md` | **スタック選定と概要ダッシュボードの設計判断。いま有効な正。** Tremor 採用の理由、盤面の構成、採らなかった図とその数値（ヒートマップ・散布図ほか）を含む |
...
（**2026-07-31 に大部分が失効。冒頭の断り書きを読むこと**）、王朝→配色スロットの対応表は
`src/lib/dynasty-colors.ts` の `DYNASTY_COLOR_SLOT`（87王朝を網羅・未知キーは throw する単一情報源）。
計画書はこれらに置き換わったので消してよい、という判断。
**当時「設計契約」としていた `site/DESIGN.md` は 2026-07-31 に失効した**（配色・フォントの正は
`src/app/globals.css`）。
...
**2026-07-31 に3件を取り下げた（下の「取り下げた条件」）。残っているのは以下。**

1. **既存のパフォーマンス対策を退行させない。** `LazyMount`・行ウィンドウイング・量子化ウィンドウ・スクロール直後150msのホバー抑制・`useTipOutlet` による state 分離（経緯: `docs/site-design/PERFORMANCE.md`。これを外すと実機 TBT 18秒・CLS 1.1 を記録した実績がある）。**旧実装のまま残っている17面では今も必須**
2. **`globals.css` の2つの回避ハックを消さない。** `html { scrollbar-gutter: stable }` と `body[data-scroll-locked][data-scroll-locked]` の二重補正打ち消し
3. **`src/components/ui/`（shadcn）と `src/components/tremor/`（Tremor Blocks）はどちらもベンダーコード。** 原則触らない（前者は `npx shadcn` の再取得で上書きされる。後者は上流からの再取得手順を持たないので、変更するなら理由をコメントに残す）

### 取り下げた条件（2026-07-31・再提案しない）

- ~~**`/kinship` の描画を1ピクセルも変えない**~~ — **ページ廃止決定のため失効**。ノード配置の凍結は
  `/kinship` ページを前提にした条件だった。`data/kinship.json` の**データ調査は継続**するが、
  可視化ページは作らない
- ~~**`/timeline` の描画を1ピクセルも変えない**~~ — **同上（ページ廃止決定）**
- ~~**`site/DESIGN.md` を設計契約とし、`npx @google/design.md lint` を通す**~~ —
  **2026-07-31 の配色・フォント差し替えで DESIGN.md が実装から外れたため失効**。
  配色の正は `src/app/globals.css`。DESIGN.md はファイル冒頭に失効の断り書きを入れて残してある
  （旧配色に戻す提案を防ぐため）

## 見た目の回帰確認（before/after 比較）

旧実装のまま残っている17面を触るときは、**実装前に必ずベースラインを撮る。**
（`/kinship`・`/timeline` の同一性確認のために作った手順だが、両ページの廃止決定後も
他の面の回帰確認に使えるので残してある。`capture-desktop.mjs` の巡回ルートに両ページが
入っているので、ルートを実際に削除したら撮影スクリプト側も直すこと。）
