...
//
// これは*ストリーム*の表示スロットであり、王朝そのものの識別色ではない。
// 群雄バンドに束ねた流れは、束ねた先を1色で示すため意図的に灰(0)にしている
// (十国の前蜀・後蜀・南唐など、系譜図では政権ごとに色が付く王朝を含む)。
// 王朝の識別色は src/lib/dynasty-colors.ts の DYNASTY_COLOR_SLOT が持つ
// (この表とは18キーで食い違うが、それは上記の描画都合の差である)。
interface StreamDef {
  label: string;
