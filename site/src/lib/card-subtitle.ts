...
// 選定の経緯・全365人分のレビュー記録は残していない。上書き表そのものがここの正。
...
export const CARD_SUBTITLE_OVERRIDES: Record<string, string | null> = {
  ...
  // 括弧なし＝デフォルトで正しい。金は逆順「漢風名（女真名）」なので、次の太祖を
  // 除いて上書き不要）。
  "liao-taizong": "耶律徳光",
  "liao-shizong": "耶律阮",
  "liao-muzong": "耶律璟",
  ...
  "liao-daozong": "耶律洪基",
  "liao-tianzuodi": "耶律延禧",
  // 金太祖: 諱データは 2026-08-01 に金史本紀の「讳旻，本讳阿骨打」へそろえて
  //「完顔旻（阿骨打）」にしたが、日本で通っているのは女真名の「完顔阿骨打」なので
  // カードはそちらを出す（他の9人は漢名が通用名なのでデフォルト規則で正しい）。
  "jin-taizu": "完顔阿骨打",
