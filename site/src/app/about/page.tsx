  ...
  return (
    <>
      ...
      >
        <Prose>
          ...
          <p>
            ...
            方針で作っています。
            {stats.emperorCount}
            名の全員について、在位の開始日・終了日は正史原文の該当箇所を引用したうえで暦換算の計算過程まで記録しています。死因と即位の経緯には判定の根拠を述べた調査メモと出典（書名・巻）を、改元・大赦・親征などの回数系8項目には数え方と判定根拠を記した調査メモを付けています（回数系は出典欄を独立させておらず、典拠にした書名・巻は調査メモの本文に記しています）。このうち
            <strong>死因と即位の経緯の出典（書名・巻）は各皇帝の個別ページ</strong>
            にも「出典」として出しています。調査メモと原文の引用はサイトの画面には出しておらず、すべて
      ...
      >
        <Prose>
          <Callout tone="muted">
            {/* GitHub Issue #70。下の段落が言っているのは「こちらが史料を読み誤って
                いるかもしれない」で、典拠である正史そのものの性格には触れていなかった。
                サイトの数値が主張しているのは「正史がそう記していること」までなので、
                その線引きを免責の先頭に置く。 */}
            <p>
              本サイトの数値は
              <strong>正史の記述を集計したもの</strong>
              で、史実を正確に反映しているわけではありません。正史の多くは次の王朝が前の王朝について編纂した歴史書で、編纂者の立場による書きぶりの偏りや、記録そのものの欠落・重複を含みます。たとえば大赦や親征の回数は「正史がそう記した件数」であって、実際に行われた回数と一致するとは限りません。数値をご覧になる際は、この点を前提としてお含みおきください。
            </p>
            <p>
