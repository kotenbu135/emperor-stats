  ...
  return (
    <>
      ...
      <Section
        ...
        description="カプセルは皇帝の在位期間（色は王朝、灰色は並立政権。復位した皇帝はカプセルが複数になり、左側面の点線でつながります）、破線枠は皇帝でないつなぎの人物（生没年または系譜からの推定で配置）、名前の「♀」は女性。朱色の矢印が継承で、ラベルは即位経路の分類と（かっこ内に）先代から見た続柄、点線＋?は史書間・史書内で記述が対立するもの（諸説あり）。灰色の実線は血縁（ラベルは父・母などの続柄）、二重線は婚姻。◆は先代を持たないグラフの根（建国など）、◇遠祖は伝説的遠祖の系譜主張を持つ人物。ノードにマウスを載せると詳細、クリックで前後のつながりを強調表示します（もう一度クリックか背景クリックで解除）。名前はドラッグで選択してコピーできます。"
      >
        <KinshipChart layout={layout} />
        <p className="mt-3 max-w-3xl text-xs leading-relaxed text-muted-foreground">
          凡例: 朱の実線矢印＝正史で裏付けられた継承（ラベルは「世襲(子)」のように即位経路の分類＋続柄）／朱の点線矢印＋?＝諸説あり／灰の実線＝血縁（ラベルは父・母・養父など。継承矢印と重なる親子関係はラベルを継承側にまとめます）／灰の二重線＝婚姻／灰の点線（カプセル左側面）＝同一人物の復位／枠線カプセル＝皇帝（高さが在位期間）／破線枠＝非皇帝（追尊皇帝・宗室など）／♀＝女性／◆建国・◆擁立＝先代を持たない政権の起点／◇遠祖＝伝説的遠祖の系譜主張（正史に記録された自称。ツールチップに内容）。同一カラム内で直後に続く継承の矢印はカプセル右側面の弓形で描きます。短い在位が密集する期間は時間軸を局所的に引き伸ばして描いています（左の年目盛りの間隔が広がっている箇所。ノードの位置と年目盛りは常に対応します）。
        </p>
      </Section>
      <Section
        id="text"
