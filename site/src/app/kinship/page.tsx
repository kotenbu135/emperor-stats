...
const LEGEND =
  "凡例: 色付きカプセル＝皇帝（高さが在位期間・2行目は「第N代・即位経路」）／破線枠＝皇帝でないつなぎの人物（生没年または系譜からの推定で配置）／丸枠＝后妃など配偶者（位置は相手方＝夫または子の父の在位に整列。生没年はツールチップ）／二重線＝皇后との夫婦、細線＝妃嬪等の生母／灰の縦横線＝親子（夫婦の連結点から子へ降りる線。母ごとに分かれ、誰と誰の間の子かを示す）／破線の親子線＝養子縁組（例: 明帝→曹芳）または実父に諸説あり（ツールチップで区別）／朱の矢印＝王朝間の交代（禅譲・簒奪など。ラベルは経路と先代との続柄）／点線＋?＝史書間で記述が対立するもの（諸説あり）。縦スケールは1年＝8pxの完全等間隔です（在位が極端に短い皇帝のカプセルは名前が読める最小の高さで描くため、実期間よりわずかに長くなることがあります。また親子の線が見えるよう、先代の在位に隣接して即位した皇帝は上辺だけをわずかに下げ、下辺＝退位年の位置は保ちます）。章の開始前に没した祖先（荘襄王など）は最上部に圧縮して配置します。伝説的な遠祖の系譜主張は皇帝カプセルのツールチップで示します。";
