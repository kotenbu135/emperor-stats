...
export function EmperorGrid({
  ...
  //   〜@min-[19rem] : 検索・王朝・区分をすべてポップオーバーへ
  //   @min-[19rem]〜 : 検索は帯へ、王朝・区分はポップオーバー
  //   @4xl(56rem)〜  : すべて帯に並ぶ（ポップオーバーのボタンは消える）
  //
  // **検索の境目 19rem(304px) は帯の内幅で、実測で置いた値**（2026-09-19）。
  // 内幅は 320px 幅の画面で272px・360pxで312px・390pxで342px・768pxで438pxなので、
  // 直前の 26rem(416px) は**どの携帯の幅でも検索窓が出ない**位置にあり、365枚・
  // 縦44,725pxの一覧でいちばん要る操作がアイコン1つ（36×32）の中に畳まれていた。
  // 帯の固定分はジャンプ117px＋絞り込み36px＋件数36px＋gap24pxの213pxで、
  // 残りが検索窓の幅になる（内幅342px→129px・312px→99px・272px→59px）。
  // **272px（＝320px幅の画面）は残りが59pxしかないので畳んだまま**にしてある。
  // 下げるときは 320/360/390/412/430px の5本で帯が折り返していないことを見ること
  // （折り返すと SECTION_NAV_H が黙ってずれる）。
  const filterControls = (
    <>
      <div className="hidden min-w-0 flex-1 @min-[19rem]/bar:block @xl/bar:max-w-[12rem]">
        <SearchField
          bare
          value={query}
          onChange={setQuery}
      ...
      <Popover open={filterOpen} onOpenChange={setFilterOpen}>
        <PopoverTrigger asChild>
          <Button
            variant="outline"
            // 狭い帯ではアイコンだけにする。文字を残すと、条件が効いたときの
            // 縮み代がジャンプのトリガーからしか出せない。
            // 読み上げ名は aria-label が持つので、文字が消えても名前は残る。
            // **文字が消える帯でも検索窓そのものは出ている**（@min-[19rem] から）ので、
            // このボタンが畳んでいるのは王朝・区分の2つだけ。
            aria-label="絞り込み"
