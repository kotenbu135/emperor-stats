    ...
    >
      ...
      <p
        className={cx(
          ...
          hasOnValueChange &&
            'group-hover:text-foreground dark:group-hover:text-gray-50',
          activeLegend && activeLegend !== name ? 'opacity-40' : 'opacity-100',
        )}
      >
        {name}
...
const ScrollButton = ({ icon, onClick, disabled }: ScrollButtonProps) => {
  ...
  // disabled になったら押下状態を捨てる。effect 内で同期 setState すると
  // 連鎖レンダリングになるため（React 19 の lint が拾う）、押下中かどうかは
  // 描画時に disabled と併せて導出する。
  ...
  return (
    <button
      ...
      className={cx(
        ...
        disabled
          ? 'cursor-not-allowed text-gray-400 dark:text-muted-foreground'
          : 'cursor-pointer text-foreground hover:bg-muted hover:text-foreground dark:text-gray-300 dark:hover:bg-gray-800 dark:hover:text-gray-50',
      )}
      disabled={disabled}
      onClick={(e) => {
        e.stopPropagation();
      ...
      >
        <ResponsiveContainer>
          ...
          >
            {showGridLines ? (
              <CartesianGrid
                className={cx('stroke-border stroke-1 dark:stroke-gray-800')}
            ...
            <XAxis
              ...
              className={cx(
                ...
                'text-xs',
                // text fill
                'fill-muted-foreground dark:fill-gray-500',
            ...
            >
              {xAxisLabel && (
                <Label
                  ...
                  offset={-20}
                  className="fill-foreground text-sm font-medium dark:fill-gray-200"
                >
                  {xAxisLabel}
                </Label>
              )}
