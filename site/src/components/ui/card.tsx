...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        // transition-transform はホバーで持ち上げるカード(トップのセクションカード等)の
        // 土台。静止カードは動かないので見た目は変わらない。
        "group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-md border border-border bg-card py-(--card-spacing) text-sm text-card-foreground transition-transform duration-150 ease-out motion-reduce:transition-none [--card-spacing:--spacing(4)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(3)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-md *:[img:last-child]:rounded-b-md",
        className
      )}
      {...props}
    />
