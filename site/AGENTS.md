...
## 系譜図（`/kinship`）は作り直し中で、まだ**登録していない**

2026-08-17 に一度作り直して配信したが、出来が公開に耐えないとユーザーが判断し数時間後に
取り下げた（Issue #174・PR #185）。**2026-08-18 に作り直した実装がこのブランチに入っている**が、
`SITE_SECTIONS`・`nav-data.ts`・`sitemap`・`capture-site.mjs` のどれにも載せていない
（`/lab` と同じ非公開の作業ページ・`robots` は `index: false`）。**見た目がユーザーの水準に
届いたと確認できるまで登録しない。** 取り下げた前の版の失敗（面積の8割が白・淡彩8%・
...
章は3つ（秦・漢 `/kinship`・三国・西晋 `/kinship/three-kingdoms-jin`・
東晋・十六国 `/kinship/eastern-jin-sixteen`・2026-08-19）。
