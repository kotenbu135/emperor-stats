...
**ただし 2026-08-13 から Web 公開を停止しており、2026-08-21 に紹介文そのものが正式に廃止された**
（ユーザー決定。残り137本は書かず、公開も再開しない — 旗を `false` に戻す提案をしないこと。
下の「再開は〜だけ」以下の記述は、決定が覆った場合のために残してある手順）。旗は `emperors.ts` の
...
- **`src/lib/seo.tsx`** — `SITE_URL`/`SITE_NAME`・`buildMetadata()`・JSON-LD 生成関数・`SITE_SECTIONS`。各ページの `metadata` はこれ経由、`layout.tsx` の `title` は template 化済み。`app/sitemap.ts`・`app/robots.ts`・`app/manifest.ts`・`app/llms.txt/route.ts` は `export const dynamic = "force-static"` が無いと `output: "export"` でビルドが落ちる。
- **`src/app/llms.txt/route.ts`** — llms.txt（生成AI向けのサイト案内）。**`public/llms.txt` に手書きで置かないこと** — 2026-09-20 にここへ移した経緯（手書き版に数字が1つも入っておらず、生成AI経由で引用されていなかった）はファイル冒頭のコメントと [../docs/site-design/GEO_AUDIT_2026-09-20.md](../docs/site-design/GEO_AUDIT_2026-09-20.md) にある。**数字はここで数え直さず `getOverviewStats()`・`getHomeHighlights()` の戻り値をそのまま文にする**（トップページと同じ集計を通すので、データを訂正すると llms.txt の数字も一緒に動く）。非業の死の区分も `reignDeath.segments[0]` から引いていて、定義は `emperors.ts` の `REIGN_DEATH_SEGMENTS` 1か所にしかない。
