...
- 既知の未対応バックログ（着手はユーザー指示があってから）: Nivo チャートの一括レンダリングによる TBT、`/emperors` ファーストビュー肖像画の LCP、a11y 2件（SelectTrigger / Nivo SVG のアクセシブルネーム）。詳細は `../docs/site-design/LAYOUT.md` の品質計測節。
