...
- **`SectionJumpNav` の `trailing` は右詰めしない**（2026-08-04）。右詰めするのは件数だけで
  `ml-auto` は渡す側（`EmperorGrid`）が持つ。箱ごと右詰めすると帯の余り（実測: ビューポート
  1920で321px・1024で142px）がジャンプとの間＝帯の中央に集まる。群を仕切る罫線と
  「時代へジャンプ」の見出しは**内幅42rem以上でだけ**出す（それ未満は群の間が8pxしかない）
- **帯の上に本文の `py-section` を残さない**（/emperors）。呼び出し側で `-mt-section` を
