...
- **縮む側（ジャンプのトリガー）には幅の下限を置く。** 条件が効くと帯の右側が太り、
  その増分を縮み代のある要素が全部かぶる（下限が無いと時代名が2文字まで潰れた）。
  **溢れてはいないので `scrollWidth` の検査では拾えない**
- 触ったら実測し直す:

```bash
npm run build && node tools/bar-audit.mjs   # 高さ48px・横溢れ・トリガー実幅・0件時 → NG: 0
```
