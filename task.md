...
- [ ] **B-5（B-4作業中に発見・未対応）**: `duration.source` 374件中12件が `quote`/`conversion` 未付与。うち3件（`tang-aidi`・`qing-shengzu`・`beizhou-jingdi`）は純粋な正史出典で quote が無いだけだが、**9件（`liang-xiaozhengde`・`beiwei-xiaozhuangdi`・`beiqi-andewang-gaoyanzong`・`beiqi-youzhu-gaoheng`・`beizhou-mingdi`・`beizhou-wudi`・`beisong-qinzong`・`jin-hailingwang`・`jin-aizong`）は page にWikipedia記事名（(ja)/(zh)付き）が正史書名と混在したまま残存**。`detect_wikipedia_sources.py` の HISTORY_KEYWORDS 部分一致（「梁書」「金史」等を含むため正史扱い）による検出漏れで、ブロック10〜12で発覚した既知の限界と同型。source.note を見る限り大半は過去に個別調査・ユーザー承認済みだが、page の整形・quote/conversion 整備・日付の原典裏付けは未了。個別ページの「在位日付の典拠」節にはこの page がそのまま表示される
