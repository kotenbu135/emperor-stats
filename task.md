...
- [ ] **次の作業: 生母（実母）の追加調査** — フェーズ0のスコープルール「実母は接続に寄与する場合のみ」をユーザー決定で転換（2026-07-24）。「誰と誰の間に生まれたか」を全皇帝で分かるようにするため実母を全域収録する（現状の実母エッジは9本のみ。約330人規模の原典調査。趙姫〔始皇帝生母・元呂不韋の姬〕もここで収録）。**未着手・開始指示待ち**
- [ ] 可視化の再設計（生母調査完了後に仕切り直し）
