...
- [x] **西晋八王の追加収録 — 完了（2026-07-25）**: 未収録だった7王（汝南王亮・楚王瑋・斉王冏・長沙王乂・成都王穎・河間王顒・東海王越）と接続ブリッジ6人（司馬防・孚・馗・瓌・泰・攸）を晋書（巻一 宣帝紀・巻三十七 宗室伝・巻三十八 宣五王／文六王・巻五十九 八王伝・巻四 恵帝紀・巻五 孝懐帝紀）で個別調査し収録（persons +13・エッジ +14）。収録のため `KINSHIP_SCHEMA.md` にスコープルール5「政変当事者」を加算的に追加（ユーザー決定・適用は八王に限る）。**残: /kinship 第2章（三国・西晋）の西晋バンドのレイアウト再調整**（`docs/site-design/KINSHIP.md` の追加調査タスク節も同時に完了へ更新すること）
