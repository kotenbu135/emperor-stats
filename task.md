...
**フェーズB（reigns[].duration 350件）: 進行中（2026-07-21着手）— 335/350件完了・残12件（`nanming-anzong`・`shun-lichengzheng`は検出スクリプトの部分一致判定で旧page値が誤って正史扱いされ350件のカウント対象からも漏れていたため、実際の完了数と350件ベースの差分は完全には一致しない。詳細下記ブロック10・11注記）**
...
- [ ] ブロック12: 劉永（`liu-yong-liang`、新末・梁王）・袁術（`yuan-shu`、仲家）2名＋隋末群雄10名（`suimo-yuwenhuaji`・`suimo-wangshichong`・`suimo-liangshidu`・`suimo-xuerengao`・`suimo-liqui`・`suimo-xiaoxian`・`suimo-lizitong`・`suimo-linshihong`・`suimo-fugongshi`・`suimo-zhucan`）の原典突合。2026-07-21ブロック11の残存確認で判明した、旧ブロック1（秦漢）・旧ブロック5（隋唐）が対象外としていた反乱・自称政権群（`docs/process/CORPUS_NOTES.md`にdeathCause/accessionRoute調査時の史料マッピング記載済み、後漢書列伝・晋書列伝等）。既存の`startDate`/`endDate`は原則変更せず、正史の干支と食い違う場合は個別に提示して承認を得る
