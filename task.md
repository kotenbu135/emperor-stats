...
残フェーズ（詳細・規模感は KINSHIP_SCHEMA.md の調査フェーズ計画を参照。進捗は kinship.json の `meta.status.phases`）:
- [ ] フェーズ1 `succession`: 継承エッジ約365本（王朝ブロック単位・原典個別確認）
- [ ] フェーズ2 `parentage`: 実父母・養父＋ブリッジ人物（追尊皇帝含む・推定100〜150人）
- [ ] フェーズ3 `interdynastic`: 王朝間縁戚クラスタ20〜30件＋系譜主張
- [ ] フェーズ4 `crosscheck`: Wikidata P22/P25/P26/P1365 突合スクリーニング
- [ ] 可視化（データ完了後）: 王朝単位ツリーから。全体グラフは大河ビューの知見を流用
