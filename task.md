...
- **OGP 画像の Content-Type 問題 — 決着（2026-07-27・対応しない）**: 2026-07-22 に curl で `application/octet-stream` 配信を再確認した上で socialsharepreview.com で検証し合格。2026-07-27 にユーザーが rakko.tools の OGP プレビューアでも正常表示を確認した。ブラウザ系レンダラは Content-Type を無視して sniffing するため実害が観測されず、**拡張子付きパス化（ビルド後リネーム＋meta 書き換え）は行わない**。以後はカードの中身（クリックされやすさ）の問題として扱い、2026-07-27 に実データの事実カード／チップを追加した（項目 4b の Phase 2 参照）
...
- **Phase 2（構造の是正）— コード側は完了（2026-07-27）**: サイドバーの空 `<h3>`×7 の解消（2-1）・皇帝個別365ページの h1→h3 レベル飛びの解消（2-2）・個別ページ→統計ページの文脈内リンク 0→4〜14本/ページ（2-3）。あわせて OGP 画像に実データの事実カード／チップを追加（2-5。Content-Type はユーザーが OGP プレビューアで実害なしと確認済みのためリネームはしない）。実装記録は `docs/site-design/IMPLEMENTATION_LOG.md`「SEO 監査 Phase 2」節
  - [ ] **2-4 Cloudflare（インフラ設定・未着手）**: Transform Rules でセキュリティヘッダ付与、Redirect Rules で trailing slash 404 と `/index.html` の二重配信を301。CSP は `Report-Only` から始める。`www` の CNAME 追加は要判断
- [ ] **Phase 3（権威と発見性）**: データセットの公開先追加（awesome list / Kaggle・HF / Wikidata P854 個別提案 / jawiki はノート提案）・構造化データのグラフ結合（3-3）。**3-2（運営者情報の明示と Person ノード）は Phase 2 と同時に完了（2026-07-27）**。Zenodo DOI・FAQPage・HowTo は含めない（いずれも既に却下・廃止済み）
...
| 項目 | 内容 | 主導 |
|---|---|---|
| 0-2 | ~~OGP Content-Type の確認~~ 完了（2026-07-27・ユーザーが OGP プレビューアで正常表示を確認。実害なしのためリネーム対応はしない） | — |
| 3-2 残 | 肖像の外部照合（Wikidata P18 突き合わせ） | 任意 |
| 3-4 | events[].source 補完（優先度中391件＋遷都58件） | 要着手判断 |
| 4b | SEO 監査 Phase 2 の残＝2-4 Cloudflare（インフラ設定）・Phase 3（データセットの公開先追加・構造化データのグラフ結合） | 要着手判断 |
