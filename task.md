...
**フェーズB（reigns[].duration 350件）: 進行中（2026-07-21着手）— 318/350件完了・残32件（うち`nanming-anzong`は旧page値「朱由崧(zh)/南明史」が検出スクリプトの部分一致判定で誤って正史扱いされ、350件のカウント対象からも漏れていたため今回の是正は残件数の減少に反映されない。詳細下記ブロック10注記）**
...
- [ ] ブロック11以降（清15・明清交替期の残り5名＝南明以外の順李自成・西張献忠・呉周2名・中華帝国ほか）の原典突合。既存の`startDate`/`endDate`は原則変更せず、正史の干支と食い違う場合は個別に提示して承認を得る
