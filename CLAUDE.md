...
- **横展開・走査で出た「残り何件」は Issue を立てず [docs/process/RESIDUAL.md](docs/process/RESIDUAL.md) に行を足す**（規則 `R-RESIDUAL-TABLE`）。Issue は片づけ方が1つに決まる問題の器で、量を持つには向かない — 2026-08-02〜03 に訂正1件から Issue が28本に増え、うち6本は同じ穴が別の場所から顔を出しただけだった（親 Issue #68 へ統合）。**Issue を立てるのは直し方・定義・設計の判断が要るときだけ**で、`gh issue create` はフックが一度止める（`EMPSTATS_ALLOW=R-RESIDUAL-TABLE:<理由>` で通す）
- **一部の規則は `.claude/hooks/guard.py` が実行の直前に止める**（コーパスへの `.{0,N}` grep・`git add -A`・裸の `git stash`・メイン会話での `emperors.json` 全体 Read・外部ホストへの一括 HTTP 取得・`gh issue create`）。**サブエージェントと Workflow エージェントにも掛かる**。逃げ道は `EMPSTATS_ALLOW=<規則ID>:<理由>` の1本だけで、理由は必須。規則の一覧と適用範囲は [docs/process/RULES.yml](docs/process/RULES.yml)
