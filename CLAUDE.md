...
| 元号名 `eraName` の転記 | **2026-08-20 に母集団を読み切って Issue #161 を close した**（643/658・残る15は原典を読んでも埋まらない側）。道具は `.claude/workflows/name-block.js` に残してあり、残量と数え分けは [RESIDUAL.md](docs/process/RESIDUAL.md) の行。第N代 `dynastyOrder` も **2026-08-20 に読み切って [#24](https://github.com/kotenbu135/emperor-stats/issues/24) を close した**（89/89政権・374在位）。判定基準は [EMPERORS_SCHEMA.md](data/schema/EMPERORS_SCHEMA.md) の `dynastyOrder` 行が正 |
