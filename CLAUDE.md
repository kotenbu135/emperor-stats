...
| 紹介文（Issue #16） | `/write-profile`（規範は [docs/process/profile-writing/README.md](docs/process/profile-writing/README.md) の1枚が正）。2026-08-04 に既存76本と旧手順書を全削除し、**2026-08-05 に原文1巡＋Web差分の2段で立て直した**。見本は書き上げた3本（`han-yuandi`・`beiwei-tuobayu`・`xiliao-renzong`）で、**削除済みの76本（git `f1311ff`）は見本にしない** |
