...
| `findings[].verdict` | ○（`value: null` のときだけ） | **空の主張がどちら向きか。** `"read-absent"` = 原文を読んで「この人にこの名乗りは無い」と決めた／`"pending"` = 原文は読み終わったが値の扱いが判断待ち／`"out-of-scope"` = **調査そのものを打ち切った**（2026-08-16 追加・読む原文がコーパスに無いケース。**この1つだけは `claims`／`basis` を持たず、代わりに `reason` が必須** — 読んでいないものに witness を求めない。`coverage.py` はこのセルを**確定率の分母から外す**ので、調査エージェントが自分で付けてよい旗ではなく**ユーザーの決定を写す場所**。機械が見るのは `reason` が空でないことと**決定の日付（YYYY-MM-DD）を含むこと**の2つだけで、**その打ち切りが本当にユーザーの判断だったかは検査していない** — 率を上げる唯一の verdict なので、ここは信用に依っていると承知して置いてある）。`coverage.py` は最初の1つだけを**不在確定**に数える（2026-08-11・オプトインなので付け忘れは過小報告に落ちる）。値のある主張には要らない。旧 `pending: true` の旗は廃止（`check_claims.py` が落とす）。**`regime-conventions.json` の `verdict`（`form-found`／`absent-by-book`／`no-form` …）とは別の欄で、値も共通ではない** — あちらは政権の書式・所在、こちらは1人ぶんの読解結果 |
