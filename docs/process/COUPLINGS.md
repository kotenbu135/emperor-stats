...
| 触るもの | 一緒に触るもの | 検査 |
|---|---|---|
| `.claude/hooks/guard.py` の規則 | `docs/process/RULES.yml` の `enforcement` 欄 | `python3 scripts/check_rules.py`（実装と台帳の突合）＋ `python3 .claude/hooks/test_guard.py`（規則を足したらケースも足す） |
