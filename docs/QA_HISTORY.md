...
- **month 精度 event 日付の太陽暦換算（2026-08-02、Issue #34）** — 1073 フィールド（機械換算 968＋個別判定 105）
...
Issue #33 の横展開スイープでこれが表面化し、`events[].date`/`startDate`/`endDate` の **968 フィールド（110 人）＋ note 1 件**を訂正した。
...
- **フェーズ5（同日）で残り 193 フィールドを 1 件ずつ原典判定**し、105 訂正・88 現行維持で決着（保留 0）。
  ...
  - **Issue #33 のスイープは検出器として使えなくなった**（フェーズ5 後も 127 件ヒットし、すべて原典で確認済みの正しい値）。
    逆写像で判定する `check_month_conversion.py` に作り直し、**訂正前 157 件 → 79 件**。残る 79 件は
    「note のどの旧暦月からも説明できない値」で、#34 の未換算とは別種
