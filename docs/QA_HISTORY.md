...
- **month 精度 event 日付の太陽暦換算（2026-08-02、Issue #34）** — 968 フィールド・110 人
...
- **除外条件も換算条件もフィールド単位で書くと、期間イベントの片端だけが動く**。前後関係が壊れた5件は訂正前後の差分検査で見つけて修復したが、
  **片端だけ換算されて旧暦月と太陽暦月が同居しているイベントが31件残っている**（C の残り208件の部分集合）。
