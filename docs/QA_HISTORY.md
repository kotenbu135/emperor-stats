# データ QA・出典整備の作業記録（2026-07-21〜）

`data/emperors.json` の出典整備・日付正規化・引用検証など、**完了したデータ品質作業の詳細記録**。
2026-07-27 に [PROJECT_STATUS.md](PROJECT_STATUS.md) が371行へ肥大したため分離した（PROJECT_STATUS は現状と進行中のものに絞る）。

利用者向けの変更履歴は [../CHANGELOG.md](../CHANGELOG.md)、機械可読な完了ブロック記録は `data/emperors.json` の
`meta.*Blocks`（`deathCauseCompletedBlocks`・`reignDurationSourceBlocks` など）にある。本ファイルはその間を埋める
「何を疑い・どう検出し・何件どう直したか」の作業記録で、同種の QA をもう一度回すときの手順書として読む。
...
- events 日付の全面 ISO 正規化＋datePrecision 語彙統一（2026-07-22、task.md 3-3 の申し送り解消）
...
- **month 精度 event 日付の太陽暦換算（2026-08-02、Issue #34）** — 1937 フィールド（機械換算 1832＋個別判定 105）
...
8つの回数系指標の `events[]` に残っていた非 ISO 日付・非標準 precision トークンを全面正規化した（ages ISO 正規化〔2026-07-21〕と同じ方式: 1件ずつ個別判定・sxtwl 換算・自己検証・適用スクリプトに旧値一致ガード・原表記は note に保全）。内訳: **3桁年ゼロ埋め378件**（51人）、**元号表記の ISO 換算250件**（18人）、**precision より浅い日付の深化79件**（9人）、**非標準 datePrecision トークン695件の解消**（bare 268件＋標準接頭辞つき装飾427件。語彙は `year`/`month`/`day`/`null` に統一＝2026-07-22 ユーザー確定、date が null なら precision も null）。
...
startDate/endDate で実確認精度が異なるイベントの表現方式を **`reigns[]` と同形式のオブジェクト `{"start": ..., "end": ...}` にユーザー確定**し、44キー（20イベント・13人）を全解消した（`data/schema/ADDITIONAL_SCHEMA.md` 1節に正式化）:

- **原典個別確認で深化（20キー・5イベント群）**: 明憲宗の靖州苗乱2件（成化三年二月丁巳=1467-03-26・成化十二年三月壬戌=1476-04-12。後者は旧 note の中略引用『三月壬子……李震大破靖州苗。』が討破を壬子の条に誤帰属していたのを発見・訂正〔実際は「三月壬子，减内府供用物。壬戌，李震大破靖州苗。」〕）、前蜀王建の元膺の乱（資治通鑑の干支 丁未〜己酉=0913-08-11〜08-13）、前秦苻健の苻菁の乱（通鑑「六月…庚辰」=0355-07-05）、南燕慕容徳の慕容達クーデター（元興二年四月・癸巳朔一致→太陽暦多数月 0403-05）。いずれも sxtwl 自己検証（干支の月内一意・朔一致・閏月確認・在位範囲）合格
- **オブジェクト精度化（24キー・24イベント）**: 後漢和帝6・安帝2・東晋元帝1・前秦苻健親征1・斉海陵王1・北魏太武帝1・閩王延羲1・遼天祚帝2・明世宗5・清穆宗4（既存日付は不変、実確認精度を側別に宣言）
- **CI 格上げ**: `check_event_date_format()` が旧警告を全キーエラーへ格上げ。オブジェクト形式の構造検査（start/end 両キー必須・同値禁止・単一日付 `date` との併用禁止）を追加。site 側は `emperors.ts` の `precisionOf()` で側別クランプに対応
...
再発防止は同時制定の「引用の取り扱い規約」＋機械ゲート（`verify_quotes.py` 照合台帳・`verify_calendar.py` 暦リプレイ、CI 組込済み）が担う（`docs/process/RESEARCH_PROCESS.md`）。各スクリプトの KNOWN_*_PENDING・KNOWN_NULL_SAID・台帳 defect は全件消化済みで現在は空。残る既知の限界はコーパス外資料65件・解釈層（機械照合不能）のみ。~~ming-huizong の lacuna 1件~~は 2026-07-23 解消: 検証当初「ローカル明史巻四に洪武31年11〜12月条が欠落」とされたが、`china-history/明史/本纪/第四章-卷四-原文.html` に当該条（「十二月癸卯…是月，赐天下明年田租之半，释黥军及囚徒还乡里」）が洪武三十一年の文脈で実在することを確認（引用・年月の帰属とも note の記載どおり）。台帳エントリは規約制定時の再バックフィルで既に status=corpus となっており、現在 lacuna は 0 件。
...
## month 精度 event 日付の太陽暦換算（2026-08-02、Issue #34）

2026-07-22 の「events 日付の全面 ISO 正規化」は**表記（ISO 形式）を揃えたが、month 精度の値の換算までは行っていなかった**。
Issue #33 の横展開スイープでこれが表面化し、`events[].date`/`startDate`/`endDate` の
**1937 フィールド＋ note 1 件**を訂正した（フェーズ1〜4 の機械換算 968＋フェーズ5 の個別判定 105＋フェーズ6 の 864）。

- **欠陥の実態**: 照合可能な month 精度 1389 フィールドのうち **1151（83%）が「旧暦の月番号をそのまま ISO の月に置いた未換算値」**。
  Issue 起票時に見えていた「旧暦十一月・十二月の年またぎ 105 件」は、年までずれるので目立った部分にすぎない。
  `ages`・`reigns`・day 精度の event は 2026-07-21〜22 に太陽暦へ換算済みで、取り残されたのは month 精度の event だけだった
- **規約**: 多数月方式（旧暦月の日数が最も多く属する太陽暦の年月）は [AI_RESEARCH_LESSONS.md](process/AI_RESEARCH_LESSONS.md) 3・9節の既決事項で、
  `ages` には適用済み。今回は「決まっている規約を event にも適用する」作業として、ユーザー承認のうえ実施した
- **自己検証**: note の「〇月〇〇（干支）」がその旧暦年月に実在するかで換算の前提を検査（照合できた 293 件中 282 件＝96% が実在）。
  **実在しない 20 件は機械換算せず除外**（史書暦と sxtwl の朔差か史料誤記。5〜6世紀の北魏に多い）。
  年が動く A は本紀キャッシュの記事順でも裏取りし、**note の月自体が本紀と食い違う 1 件**（`hou-han-huandi` の叔孫無忌の乱＝蜂起は十一月）を note ごと訂正
- **触ってはいけない2種**（適用条件の根拠になった発見）:
  - note に複数の旧暦月が出る期間イベントは、ISO 月がどれに対応するか機械では決まらない（記述順から決まるものだけ扱った）
  - **日に実値がある month 精度フィールドは、既に太陽暦の日まで換算済みのことがある**（例 `wudai-houliang-modi` の `0914-09-23`＝乾化四年九月の実日付）。
    多数月方式で月を動かすと改悪になるため、日がプレースホルダ（`-01` か月末）であることを必須条件にした
- **結果**: 年またぎ 105 フィールドは決着（97 訂正・6 は現行値が正しい・2 は原典で月を確定できず保留）。月だけのずれは 1008 → 208。
  `validate_emperors.py` 0 errors、`verify_calendar.py` 0 errors
- **既存ゲートに検出力が無かった**: `check_event_reign_range`・`check_bce_event_years` は `ev["date"]` しか見ておらず、
  今回の対象の大半（`startDate`/`endDate`）を検査していない。**イベントの前後関係（`startDate` ≦ `endDate`）を見るゲートも無い**。恒久チェックは残タスク
- **除外条件も換算条件もフィールド単位で書くと、期間イベントの片端だけが動く**。前後関係が壊れた5件は訂正前後の差分検査で見つけて修復した。
  同一イベントの他端が実日付を持つときに多数月方式を当てると矛盾する型もあった（`jin-huidi` の親征）
- **フェーズ5（同日）で残り 193 フィールドを 1 件ずつ原典判定**し、105 訂正・88 現行維持で決着（保留 0）。
  これで **note に旧暦月がある month 精度フィールドの未換算は解消**し、月だけのずれ 208 件・片端混在 31 件・保留 2 件・除外 22 件はすべて片付いた。
  合計の訂正は 1073 フィールド（機械換算 968＋個別判定 105）
  - **判定の向きを逆にしたのが要点**。「現在値が未換算か」を当てにいくと、**閏月のある年で旧暦月→太陽暦月が2か月ずれる**ため
    （旧暦35年は閏三月があるので旧暦六月→`0035-08`）換算済みの値を未換算と誤認する。
    「その端点が指すべき旧暦月を原典で決める → 多数月方式で写す → 現在値と比べる」に統一した
  - **干支不在で除外した 22 件は、原典では月ラベルが原文どおり**だった（後漢書「三年春正月己未，大赦天下」など）。
    史書暦と sxtwl の朔・置閏の差なので、**month 精度の値は月ラベル基準で換算する**（干支は日精度の情報であり月精度の換算根拠にしない）
  - **Issue #33 のスイープは検出器として使えなくなった**（フェーズ5 後も 127 件ヒットし、すべて原典で確認済みの正しい値）。
    逆写像で判定する `check_month_conversion.py` に作り直した
- **フェーズ6（同日）: `YYYY-MM` 形式 2464 フィールドが全フェーズの正規表現から漏れていた**。
  フェーズ1〜5 と #33 のスイープはすべて `^\d{4}-\d{2}-\d{2}$` を要求しており、**month 精度の主流表記だった
  `YYYY-MM`（日を持たない表記）は Issue 起票時から一度も検査対象になっていなかった**。同じ方法で **864 フィールドを訂正**
  （単一月 603・記述順 232・個別判定 29）。**満洲国康徳元年（1934年）の恩赦は太陽暦なので換算しない**と判定した
  （清朝滅亡後のレコードに旧暦換算を当ててはいけない）。訂正総数は 1937 フィールド
- **検出器（両表記対応）での前後**: 2759 件中 **1016 件 → 156 件**が「note のどの旧暦月からも説明できない値」。
  残る 156 件は #34 の未換算とは別種（note が終了月しか書いていない場合と、値そのものの誤りが混在）

詳細・残タスク・スクリプトは [docs/qa/issue34-event-date-2026-08-02/FINDINGS.md](qa/issue34-event-date-2026-08-02/FINDINGS.md)。
