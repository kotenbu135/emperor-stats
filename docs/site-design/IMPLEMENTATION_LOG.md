...
Phase 2 以降（サイドバーの空 `<h3>`・皇帝個別ページの見出しレベル飛び・個別365ページ→統計ページの文脈内リンク・Cloudflare のセキュリティヘッダと URL 正規化・OG 画像の Content-Type）はこのブランチに含めない。計画上も別フェーズで、うち2件はコードでなくインフラ設定の変更。
