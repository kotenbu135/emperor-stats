...
> **⚠️ 2026-07-31 の再構築で一部が失効。** 旧実装のまま残っている17面については今も有効な記録だが、
> 以下は**現状の説明として読まないこと**。
>
> - **配色に関する記述すべて**（水墨文人パレット・`#F5F1E8`・`--seal #A6321C`・「面は明度で3段」・
>   スクロールバー `#c9bda8`・サイドバー `#ede7d8` と宣紙色の対比）— 配色は全面差し替え済み。
>   正は `site/src/app/globals.css`
> - **トップページに関する記述すべて**（8セクションカード・「収録の概況」・`max-w-4xl` 中央寄せ）—
>   新トップは `components/home-v2/`。旧実装 `components/home/home-panels.tsx` は死んだコード
> - 「メニュー構成案の全項目が稼働する状態になった」— `/timeline`・`/kinship` はメニューから外れた
...
### 検証（7観点の並行検証＋敵対的再検証）で見つけて直したもの

実装後に7観点（数値・見出し・リンク・アコーディオンの実機挙動・構造化データ・OGP画像の目視・回帰）を並行で検証し、出た指摘をさらに別のエージェントに反証させた。直したのは以下。

- **`aria-label` は可視テキストを置き換える（WCAG 2.5.3 Label in Name / Level A）**。`StatLink` に `aria-label="死因別の分布を見る"` を付けていたが、これだとアクセシブル名から可視テキスト「病死」が消え、音声入力で「病死」と言っても操作できない。説明は `sr-only` の追記（「病死（死因別の分布を見る）」）で足す形に変えた。**リンクの説明を足したいときに `aria-label` を使わない**こと。
- **OGP画像で長い文字列が2行になり下の要素と重なる**。皇帝名だけでなく**在位行**も踏んだ（宣統帝「在位 1908–1912年 / 1917年 / 1934–1945年（14年256日）」が「14年256」「日）」で分断）。全角1em・ASCII 0.55em で幅を見積もって1行に収まるサイズを逆算する `fitFontSize` を両方に使う。
- **王朝の同率1位を黙って1件に丸めていた**。`/dynasties` の事実カードだけ `reduce`/`leaders[0]` で先頭を取っていた（皇帝側は `leaderLabel` が同率を表記する）。「◯◯ら2王朝」の表記を足した。現データに同率は無いが、データが動いたときに静かに嘘になる。
- **`/about` の説明文が3箇所に分散していて OGP だけ取り残された**。3-2 で `metadata` と `PageHeader` には「運営者情報」を足したのに `opengraph-image.tsx` の直書きが旧文のままだった。OGP は画像に収まる短縮版が要るので定数を共有できず、**対で直すこと**をコメントに明記した。
- **本文の記述がデータの実態より強かった**。運営者節に書いた「回数系の各項目にも…出典（書名・巻）を付けています」は誤りで、回数系8項目の `count` は `(confidence, count, events, note)` のみで `source` を持たない（典拠は note 本文に記述。書名・巻の言及があるのは 1812/2920）。「回数系は出典欄を独立させておらず、典拠にした書名・巻は調査メモの本文に記しています」に改めた。在位日付の引用・暦換算は 365/365（全在位期間）で記述どおり。
- 「のちに復位」から `/reign#restoration` への導線も足した（復位は順位指標ではないので上のリンクの仕掛けに乗らず、8名だけ復位者一覧へ辿れなかった）。

**ついでに直した既存問題**: トップページの概況4値（`OverviewFigures`）が `<dl> > <div>` の中に `dt`/`dd` と並べて `<p>`（注記）を置いており、HTML として不正で axe の `definition-list`（serious）に当たっていた。**`<dl>` 直下の `<div>` に置けるのは `dt`/`dd` だけ**。注記を `<dd>` の中へ移し、値と注記を `block` の `span` にして従来と同じ縦積みを保っている（見た目は不変）。今回の 2-1〜2-5 とは無関係の既存問題で、ユーザーの指示で同じブランチで対応した。axe は `/`・`/reign`・皇帝個別2件・`/about` で violations 0 になった。
