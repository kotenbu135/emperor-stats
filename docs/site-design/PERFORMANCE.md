...
> **⚠️ ここに載っている計測はすべて旧実装（Nivo）に対するもの。**
> 2026-07-31 に作り直したトップページ（Tremor + recharts）は**一度も計測していない**。
> 「全9ページ計測済み」「バックログなし」といった記述を**新トップにまで広げて読まないこと**。
> 対策そのもの（`LazyMount`・行ウィンドウイング・`useTipOutlet` 分離など）は、
> **旧実装のまま残っている17面では今も必須**。新トップは `LazyMount` を使っていない
> （世紀チャートが棒23本で、これらの対策が対象としてきた365行チャートとは規模が違うため）。
