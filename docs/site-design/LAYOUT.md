...
- **配色**: 墨の濃淡を基調にした抑制的な配色。朱色は印章的なワンポイント（ランキング1位バッジ・見出し脇のアクセントバー・現在地ナビ等）のみに限定使用。**ランキング棒グラフは当初この朱の単色で塗っていたが、2026-07-27に王朝色へ移行した**（下記「王朝色システム」節）。朱をチャートの塗りに使わないことでこの限定使用の原則に戻っている。
...
- **王朝・時代で見る**（`/dynasties`）: 平均在位年数（皇帝数をラベル併記）と死因の内訳（検証済み8色死因パレットの積み上げ横棒、`enableTotals`で合計表示）。**集計単位トグル（王朝別/時代別）**と時代順ソートを装備。実装当初は平均在位年数が朱の単一系列、積み上げのセグメント間はsurface色`borderWidth:1`で区切っていたが、**2026-07-27に平均在位年数は1棒＝1王朝の王朝色（時代別集計時のみ`--series-1`単色にフォールバック）、積み上げのセグメント区切りは各カテゴリ色の82%濃度の輪郭に変更**（下記「王朝色システム」節）。この画面は上下2チャートが同じ色の言語になっているかを見る基準面になる。
