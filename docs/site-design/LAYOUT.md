...
- **フルイドな大サイズは `--text-page-title`**（`clamp(1.5rem, 1.142rem + 1.527vw, 1.875rem)`）。h1（`PageHeader`）とトップの概況の大数字が対象で、従来の `text-2xl → md:text-3xl` と両端は同値。DESIGN.md のタイポスケール6段に新しい段を足したわけではない。
