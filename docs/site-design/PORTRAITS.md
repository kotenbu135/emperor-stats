...
| **現在の確定数** | **145人** |
| 画像なし（本調査の除外分を含む・現在） | 219人 |
| 記事自体が見つからない | 0人 |

- 全364名がzh.wikipediaの何らかの記事にマッチ（取りこぼしなし）
- 145名分の詳細（照合したタイトル・画像ファイル名）は [`portraits-candidates.json`](./portraits-candidates.json) に記録
- 残り219名は本調査では**「画像なし」として一旦保留**。実装時に個別確認する
...
**この1件はダンプ照合（`page_image_free`）の産物ではない。** `ming-yizong` は2026-07-17の機械照合では画像なし側にいた（zh.wikipedia の記事にリード画像フラグが立っていなかった）。そのため `manifest.json` の `matchedWikiTitle` と `portraits-candidates.json` の `matchedTitle` は **`null`** にしてある — 照合で当てたタイトルが存在しないため。**「画像なし」219名の中には同様にCommons側には画像があるものが残っている**（PORTRAITS.md 末尾の「未確認・今後の注意点」で予告していたとおり）ので、見つかり次第この手順で足せる。

取得は既存と同じく Commons のファイル名MD5からURLを算出して幅500pxサムネイルを直接取得（1リクエスト）、ライセンスは Commons Action API の `extmetadata` を1リクエストで確認した。

## 画像ダウンロード（2026-07-17）

機械照合の168名分の画像本体を取得し、`data/images/portraits/` に配置（目視確認で13名分、CC BY-SA方針除外で2名分を除外、この時点で**153ファイル**。2026-07-21の目視QAでさらに3件除外して150ファイル、2026-07-31にさらに6件除外して144ファイル、2026-08-01に1件追加し現在は145ファイル）。
