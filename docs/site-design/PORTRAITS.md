...
サイトに皇帝の肖像画を掲載する構想について、**実際に画像が取得できるか**をdata/emperors.json収録の364名全員（当時の全収録数。2026-07-20追加の唐哀帝は本調査の対象に含まれない）で確認した記録。**画像の実装自体はデータ確定後（[CLAUDE.md](../../CLAUDE.md)参照）**に着手する。
