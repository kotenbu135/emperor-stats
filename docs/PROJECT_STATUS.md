...
- [ ] **改元回数** — `eraChangeCount` — グループ2・進行中（328/364名、詳細は下記グループ2参照）
- [ ] **大赦回数** — `amnestyCount` — グループ2・進行中（328/364名、改元回数と同時調査）
- [ ] **立后（皇后冊立）回数** — `empressInstallationCount` — グループ2・進行中（328/364名、改元回数と同時調査）
- [ ] **皇太子廃立回数** — `crownPrinceDepositionCount` — グループ2・進行中（328/364名、改元回数と同時調査）
...
> ⚠️ 上記4項目（改元・大赦・立后・皇太子廃立）は364人中328人分のデータ投入が完了しているが、`meta.status.phases` の該当キーはまだ `"not-started"` のまま（2026-07-16時点で未更新）。**全ブロック完了時に一括で `"completed"` に更新すること。**
...
現在の完了計：328/364名。次は明・清ブロック（詳細な進捗・特筆事項はメモリ `group2-count-progress` および各 `group2-block-*` を参照）。
