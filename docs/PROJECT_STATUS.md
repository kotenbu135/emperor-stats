...
- [ ] **即位経路** — `accessionRoute` — 365人全員の初回調査は完了済みだが、**2026-07-26 に多軸化（4軸＋2補助）へ移行中で `in-progress` に戻した**。移行の過程で原典と食い違う旧判定が見つかり訂正しているため（下記）。定義は [ADDITIONAL_SCHEMA.md](../data/schema/ADDITIONAL_SCHEMA.md) 1節、進捗は `meta.status.phases.accessionRoute.axesMigration`

  **多軸化で判明した旧判定の誤り（訂正済み）**: 血縁の近さを理由に `世襲` としていたが、原典には先帝の意思を示す記述がなく擁立の主体が明記されていた5件 — 晋愍帝（閻鼎ら臣下が長安に奉立）・東晋康帝（庾冰が成帝の実子を退けて弟を擁立。康帝紀「乃言國有強敵，宜立長君，遂以帝為嗣」）・東晋哀帝・東晋廃帝（ともに崇徳太后の令・詔で迎立）・東晋恭帝（劉裕が遺詔を偽って擁立）。いずれも `note` は主導者に触れていないか（哀帝・廃帝）、逆に `note` が「群臣に擁立され」と書きながら `category` が `世襲` だった（愍帝）。**`note` と `category` の突き合わせだけでは見つからない型があり、ブロックごとの原典再読で拾っている**
