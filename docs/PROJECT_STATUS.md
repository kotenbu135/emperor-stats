...
**2026-07-18、全12項目・364人全員分のデータ収集・検証が完了し、`meta.status.overall` は `"completed"` になりました。** `"in-progress"` の間はデータ未確定としてサイト実装に着手しない方針でしたが、この制約は解除されています（着手可能な状態ですが、本稿執筆時点ではまだ着手していません）。
