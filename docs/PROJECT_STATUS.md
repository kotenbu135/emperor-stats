...
- **残っているのは `/`（新実装）・`/emperors`（改修中）・`/emperors/[id]`・`/about`（新実装）・`/database`（新実装）**。`/emperors/[id]` と外側のシェル（サイドバー・ヘッダー・フッター）は旧実装のまま
...
- **最終的なページ構成を4ページに確定**（ユーザー決定）: 概要ダッシュボード（完成）・皇帝一覧（改修中）・**データベース（新規・全365名を表形式で・TanStack Table でソート/検索/絞り込み・主要8列）**・このサイトについて（実装済み）。皇帝個別 `/emperors/[id]` の365ページは別枠で現行のまま残す
...
- **現存するページ**: 概要ダッシュボード（`/`・新実装で完成）・皇帝一覧（`/emperors`・改修中）・データベース（`/database`・新実装）・皇帝個別（`/emperors/[id]`・365ページ・旧実装）・このサイトについて（`/about`・新実装）
