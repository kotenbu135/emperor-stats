...
- [ ] **親征回数** — `personalCampaignCount` — グループ3・調査中（秦・前漢・玄漢後漢・新末群雄・三国時代晋・南北朝・五胡十六国・隋唐・五代十国ブロック完了）
- [ ] **反乱鎮圧回数** — `rebellionSuppressionCount` — グループ3・調査中（秦・前漢・玄漢後漢・新末群雄・三国時代晋・南北朝・五胡十六国・隋唐・五代十国ブロック完了）
- [ ] **被反乱回数** — `rebellionSufferedCount` — グループ3・調査中（秦・前漢・玄漢後漢・新末群雄・三国時代晋・南北朝・五胡十六国・隋唐・五代十国ブロック完了、算入基準を2026-07-16に訂正・秦〜新末群雄35名監査済み、三国時代晋以降のブロックは訂正済み基準を最初から適用）
- [x] **遷都回数** — `capitalRelocationCount` — グループ4・364人全員完了
- [ ] **即位時年齢・没年齢** — `ages` — グループ5・悉皆調査に格上げ（2026-07-17）・隋唐ブロック（48名）・五代十国ブロック（34名）完了、241/364人処理済み、残り123人を専用ブロックで調査
...
グループ2と同じ王朝ブロック順・史料マッピングを踏襲。隋・唐ブロックからは、子Agentのトークン削減のためグループ3とグループ5（ages）を1エージェント1回に合算する方式を試験導入し、五代十国ブロックでも継続した（0エラー、id自己申告のズレも0件、count/events不一致・スキーマ外キー混入も最終的に0件）。詳細な進捗・特筆事項はメモリ `group3-count-progress`・`group5-ages-method`・`group3-group5-merged-pass-decision` を参照。
