...
- [ ] **親征回数** — `personalCampaignCount` — グループ3・調査中（秦・前漢・玄漢後漢・新末群雄・三国時代晋・南北朝・五胡十六国ブロック完了）
- [ ] **反乱鎮圧回数** — `rebellionSuppressionCount` — グループ3・調査中（秦・前漢・玄漢後漢・新末群雄・三国時代晋・南北朝・五胡十六国ブロック完了）
- [ ] **被反乱回数** — `rebellionSufferedCount` — グループ3・調査中（秦・前漢・玄漢後漢・新末群雄・三国時代晋・南北朝・五胡十六国ブロック完了、算入基準を2026-07-16に訂正・秦〜新末群雄35名監査済み、三国時代晋・南北朝・五胡十六国ブロックは訂正済み基準を最初から適用）
- [x] **遷都回数** — `capitalRelocationCount` — グループ4・364人全員完了
- [ ] **即位時年齢・没年齢** — `ages` — グループ5・悉皆調査に格上げ（2026-07-17）・364人中213人分はグループ1〜3の副産物で判明済み、残り151人を専用ブロックで調査
...
グループ2と同じ王朝ブロック順・史料マッピングを踏襲。詳細な進捗・特筆事項はメモリ `group3-count-progress` を参照。
...
- ブロック着手順：明・前漢・後漢・清の4大ブロック（計55人）を優先し、その後グループ2/3と同じ12ブロック区分で残り（約96人）を処理する。
