...
- **【note に旧暦月がある分は解消済み・残タスクあり 2026-08-02／Issue #34】month 精度の `events[].date` が旧暦の月番号のまま置かれていた**: 紹介文（Issue #16）執筆中に出た note と原典の食い違い（Issue #33）を調べる過程で機械スイープにより検出し、同日 **event 日付 1073 フィールド（機械換算 968＋個別判定 105）＋ note 1 件を訂正**した（記録: [docs/qa/issue34-event-date-2026-08-02/FINDINGS.md](qa/issue34-event-date-2026-08-02/FINDINGS.md)）。
  ...
  - **Issue #33 のスイープはもう検出器として使えない**（フェーズ5 後も 127 件ヒットし全部が正しい値）。逆写像で判定する `check_month_conversion.py` に作り直した。**訂正前 157 件 → 79 件**
  - **残タスク**: note のどの旧暦月からも説明できない **79 件**（`check_month_conversion.py` が一覧を出す。note が終了月しか書いていない場合と値の誤りが混在）、note に旧暦月が無い 969 件、day 精度の個別誤り3件（別 Issue 候補）、`startDate` > `endDate` 34 件（27 件は BCE 年規約に絡む別問題・7 件は精度違いの文字列比較）、恒久チェック（`check_event_reign_range` は `date` しか見ておらず `startDate`/`endDate` を検査していない。イベントの前後関係の検査も無い。`check_month_conversion.py` の CI 組み込みは 79 件を片付けてから）
