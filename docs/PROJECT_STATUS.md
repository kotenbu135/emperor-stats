...
| 2026-08-13 | **紹介文（Issue #16）の Web 公開を一時停止した**（ユーザー指示）。旗は `site/src/lib/emperors.ts` の `PROFILES_PUBLISHING_PAUSED` 1つで、`true` の間は 228本すべてが**サイトに出ない**（`<meta description>`・Person JSON-LD も機械生成文に戻る）。**データ `data/emperor-profiles.json` は消していない**・**執筆も止めていない**ので、再開は定数を `false` にして再ビルド・再配信するだけ → **2026-08-21 に正式廃止で決着**（下の行） |
| 2026-08-21 | **紹介文（Issue #16）を正式に廃止した**（ユーザー決定）。残り137本は**書かない**・Web 公開も**再開しない**（`PROFILES_PUBLISHING_PAUSED = true` を恒久化）。既存228本の `data/emperor-profiles.json`・断片台帳・規範 `profile-writing/README.md`・道具（`profile-*` エージェント・`/write-profile`）は**削除せず現状のまま凍結**。残量表の紹介文系7行（ルビの読み・諱1字の第三者指し・数え年の1歳割れ・外部レビュー当て・断片無し47本・廟号指し138箇所・観点1〜9）はこの決定で打ち切り |
...
v3 で持ち越した宿題は「`dynastyOrder`（第N代）が53政権で未調査」と「kinship persons の政権帰属（`regimeId`）」の2件。前者は 2026-08-03（Issue #69）に**未調査の欄を落として** [残量表](process/RESIDUAL.md) の行（198在位／190人／53政権）にし、**2026-08-20 に読み切って残 0**（89/89政権・374在位に欄・数値353・`null` 21。[#24](https://github.com/kotenbu135/emperor-stats/issues/24) は close）。後者は別 Issue 扱いのまま。
...
| 第N代（Issue #24） | 在位 | 374 | 353 | 21 | 0 | 0 | 100.0% |
...
| 5 | ~~改元 event の元号名 `eraName` の転記~~ **2026-08-20 に母集団を読み切った** | ~~221 event~~ **残 15**（原典を読んで埋まる側は 0） | ~~Issue #161~~ **[#161](https://github.com/kotenbu135/emperor-stats/issues/161) は 2026-08-20 に close**（量は残量表） | **658 event 中 643 が転記済み。** 15ブロック・検証エージェント 229 体で読み切った。残 15 の内訳は「ユーザー決定で空のまま11／値がそもそも無い2／不在確定1／底本の限界1」で、**着手前に数え分けた3種（名がそもそも無い・底本の字が壊れている・袁世凱）はいずれも実在した**。副産物（改元 event 自身の取りこぼし・日付の疑い 20前後・字体25件）は**欄が違う**ので残量表の別の行 |
...
| 7 | ~~紹介文の残り~~ **2026-08-21 に正式廃止（ユーザー決定）で決着** | ~~137本~~ 0本（書かない） | 器は不要になった | **執筆も公開もしない**（節目の表の 2026-08-21 行）。既存228本のデータ・規範・道具は削除せず凍結。残量表の紹介文系7行も同日に打ち切り |
...
| 9 | 「完了」と称している項目の判別不能 | 635セル | 残量表 | 生年月日195・即位時年齢189・没年齢96・没年月日76・即位日45・退位日34（`coverage.py` の実測。**第N代は 2026-08-20 に読み切って 0 になり、この内訳から外れた**）。**誤りではなく「完了の主張が機械では確かめられない」** |
| 10 | サイト | — | Issue #94（モバイル回遊）・#81（肖像画が無い211人） | データ作業と独立に進められる |

**1・2・3・4・5が閉じた**（1・2は 2026-08-13、**4は 2026-08-14**、3は 2026-08-16、**5は 2026-08-20**）。
**5（元号名）で名前系の Issue 系列 #37 → #126 → #161 がすべて閉じた** — 残った15件は
判断待ちではなく「原典を読んでも埋まらない」と1件ずつ確かめた側で、
**量の置き場は残量表の行だけ**になった。
