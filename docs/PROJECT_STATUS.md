...
- **残る警告（スコープ外）**: startDate/endDate の深さが precision より浅い44件（開始のみ日精度で終了が年精度等、単一トークンで混在精度を表現できない設計課題。方式確定後に対応）・deathDate > endDate 49件（在位終了事由フィールド新設が前提、2026-07-21 ユーザー判断で見送り）
