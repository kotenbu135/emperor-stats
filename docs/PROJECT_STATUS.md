...
| 諡号（短縮呼称・Issue #126） | 人 | 365 | 151 | 105 | 109 | 70.1% |
...
| 6 | 名前欄の検証段（別コンテキスト） | 134人＋2026-08-13 の欄＋**2026-08-14 の43人** | 残量表 | `R-VERIFY-TIER`。**行が無いブロックは黙って未検証のまま残る**ので、ブロック行を立ててから回す。2026-08-14 のぶんは厚みが書で割れる（本紀が独立する北史3・新元史1は1体、列傳・紀傳・編年に依存する舊唐書23・明史8・小腆紀傳4・資治通鑑4は3体） |
...
**次は 3**（867セル読んで実在は約90セル）。ただし**「短縮諡 約51」は 2026-08-13 の決定より前の見積もり**なので、
着手前に数え直す — 反例10件のうち8件は「短縮呼称をどう決めるか」の問題で、その決定
（「全長形しか無い政権では作らない」）が同日に出ているため、**約51のうち何件かは値ではなく
`read-absent` に落ちる**。
