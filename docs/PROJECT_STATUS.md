...
- **【主要部分は解消済み・残タスクあり 2026-08-02／Issue #34】month 精度の `events[].date` が旧暦の月番号のまま置かれていた**: 紹介文（Issue #16）執筆中に出た note と原典の食い違い（Issue #33）を調べる過程で機械スイープにより検出し、同日 **event 日付 968 フィールド（110 人）＋ note 1 件を訂正**した（記録: [docs/qa/issue34-event-date-2026-08-02/FINDINGS.md](qa/issue34-event-date-2026-08-02/FINDINGS.md)）。
  ...
  - **残タスク**: C の残り 208 件（うち 31 件は片端だけ換算されて旧暦月と太陽暦月が同居している）・保留 2 件・除外 20 件の個別判定、note に旧暦月が無い 969 件、day 精度の個別誤り3件（別 Issue 候補）、恒久チェック（`check_event_reign_range` は `date` しか見ておらず `startDate`/`endDate` を検査していない。イベントの前後関係の検査も無い）。**フェーズ2で使った「note の最初の月＝開始・最後の月＝終了」という規則は今回のために決めたもので検証が薄い**（353 件中 122 件しか干支で裏取りできていない）
