...
- [ ] **親征回数** — `personalCampaignCount` — グループ3・調査中（秦・前漢・玄漢後漢・新末群雄・三国時代晋・南北朝・五胡十六国・隋唐・五代十国・宋遼西夏金・元北元元末群雄・明ブロック完了）
- [ ] **反乱鎮圧回数** — `rebellionSuppressionCount` — グループ3・調査中（秦・前漢・玄漢後漢・新末群雄・三国時代晋・南北朝・五胡十六国・隋唐・五代十国・宋遼西夏金・元北元元末群雄・明ブロック完了）
- [ ] **被反乱回数** — `rebellionSufferedCount` — グループ3・調査中（秦・前漢・玄漢後漢・新末群雄・三国時代晋・南北朝・五胡十六国・隋唐・五代十国・宋遼西夏金・元北元元末群雄・明ブロック完了、算入基準を2026-07-16に訂正・秦〜新末群雄35名監査済み、三国時代晋以降のブロックは訂正済み基準を最初から適用）
- [x] **遷都回数** — `capitalRelocationCount` — グループ4・364人全員完了
- [ ] **即位時年齢・没年齢** — `ages` — グループ5・悉皆調査に格上げ（2026-07-17）・隋唐ブロック（48名）・五代十国ブロック（34名）・宋遼西夏金ブロック（52名）・元北元元末群雄ブロック（18名）・明ブロック（16名）完了、実測268/364人`ages`フィールド保有（残り96人、前漢14・清14・後漢14等）※過去の累計値はブロックサイズの単純加算による見積りで実際の重複分（既に判明済みだった人物）を控除していなかったため、2026-07-17にjqでの実測値に置き換えて訂正
...
364/364名全員完了（2026-07-16）。詳細な進捗・特筆事項はメモリ `group2-count-progress` および各 `group2-block-*` を参照。

### グループ3（親征・反乱鎮圧・被反乱）— 調査中
...
グループ2と同じ王朝ブロック順・史料マッピングを踏襲。隋・唐ブロックからは、子Agentのトークン削減のためグループ3とグループ5（ages）を1エージェント1回に合算する方式を試験導入し、五代十国・宋遼西夏金・元北元元末群雄・明ブロックでも継続した（宋遼西夏金は52名中エラー2件〔nansong-gaozongのcount/events不一致、范汝為の乱の計上漏れ〕を検出し単発Workflowで修正、元北元元末群雄は18名中0エラー、明は16名中1件〔ming-yingzongのcount/events不一致〕を検出し既存データからの転記で単発修正、最終的にすべてid自己申告のズレも0件・suppression>suffered逆転異常も0件）。詳細な進捗・特筆事項はメモリ `group3-count-progress`・`group5-ages-method`・`group3-group5-merged-pass-decision`・`group3-block-songliaoxixiajin`・`group3-block-yuan-beiyuan-yuanmo`・`group3-block-ming` を参照。
...
### グループ5（即位時年齢・没年齢）— 調査中（2026-07-17悉皆調査に格上げ）
