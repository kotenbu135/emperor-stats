...
| 諡号の段（Issue #126） | 人 | 365 | 177 | 146 | 42 | 88.5% |
...
| 4 | 諡の段の未走査書 | ~~77人~~ **42人・7書** | 残量表 | 書1冊ずつの走査。**2026-08-13 に漢書14と新五代史の十国20を閉じた**（諡の段 79.2%→**88.5%**）。残るのは舊唐書21・明史の元末群雄8・小腆紀傳4・資治通鑑の西燕4・北史の西魏3・新元史1・**中華帝国1＝袁世凱は正史が無く走査の対象外**。次に大きいのは舊唐書。**この走査が既存 `read-absent` の誤りを3件出した**（梁武帝・梁元帝・齊和帝＝「谥」の字を使わない授与。取りこぼし率 147人中3人＝2.0%）で、同型の穴が短縮呼称・全長形・廟号の `read-absent` 側に残っている（残量表の行） |
...
**1・2・4の一部が 2026-08-13 に閉じた**（諡号（短縮呼称）の確定率 64.4%→70.1%・諡の段 77.8%→**88.5%**）。
**4は書1冊ずつ進む** — 漢書14と新五代史の十国20を閉じて 77人→42人。
ただし**2で効いた「政権ぐるみ」の安さは段には効かない**（`absent-by-book` で打ち切れるのは
書がその欄そのものを持たない場合だけで、段は人物ごとに条の有無が割れる）。
効いているのは**走査が書単位で済む**ことのほうで、1冊読めばその書の全員が同時に片付く。
**次は 4 の続き（舊唐書21）か 3**（867セル読んで実在は約90セル）。
3の「短縮諡 約51」も、残る15書のうち何冊かは走査で消える見込み。
