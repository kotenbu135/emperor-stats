...
（元帝・長安・儒学・皇太子には振らない）。`description` には振らない。
記法は `｜親文字《ルビ》` で `｜` を省略しない。
...
**`data/profile-ruby-lexicon.json`（433語）に載る語は必ず振る** — 干支60（「丙寅」）・
官職・制度・地名・故事。ルビ無しなら断片のゲートも CI も落ちる。新しい難読語に
振ったら辞書へ足す（帝号・廟号と1字の人名断片は載せない）。
