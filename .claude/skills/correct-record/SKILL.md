...
**`--check-books` は「名乗るどの書にも引用が1断片も無い」ユニットが残っていればエラー（exit 1）です**
（既知の残件は `data/quote-refs.json` の `bookAllow` に理由つきで載せてあるぶんだけ除外されます）。
**出力を読んで、自分が今回足した行が出ていないか**を見ます。note に書く書名は照合器が実ファイルと突き合わせ
