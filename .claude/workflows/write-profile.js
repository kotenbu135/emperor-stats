...
function writePrompt(id) {
  return withNote([
    ...
    `6. python3 scripts/reapply_profile_ruby.py ${workDir}/${id}.json --write`,
    '   （辞書の語と自分が振った語を**全出現**へ写す。2回目以降も振る・手で数えない）',
    `7. python3 scripts/check_profile_fragment.py ${workDir}/${id}.json --strict`,
    ...
    '- **人物名は素材冒頭の「本文で使う名前」をそのまま使う**（2026-08-07 ユーザー決定）。',
    '  原文は諱1字で人物を指すが、「垂は」「勒は」は日本語として通用しない',
    '  （既存148本のうち91本・延べ922箇所がその形で、同日まとめて直した）。',
    '  **姓を繋げれば済む話でもない**ので自分で組み立てない（耶律阿保機・クビライ）。',
    '  **本人の名前にもルビを振る**。例外は諱そのものを話題にしている文だけ',
    '  （「諱は禅、字は公嗣」）。ゲートが落とす',
    '- ルビは固有名詞と中国史の語（人名・**帝号・廟号**・元号・地名・官職・制度・故事・干支）。',
    '  一般語（「皇太子」「儒学」）には振らない。description にも振らない。',
    '  **name-readings.json を先に grep しない**（違えばゲートが正解を出す）。',
    '  **初出だけに振らない** — 手順6が全出現へ写す。data/profile-ruby-lexicon.json に',
    '  載る894語は必ず振られる。**辞書へ足すのは2冊以上に出る語だけ**',
    '  （1語足すごとに残り318人へその読みが固定される）',
    '- **読みはユーザーに確認しない**（2026-08-06 決定）。ただし**中国史の語に日本の元号の',
    '  読みを当てない** — 「元和」は げんな（日本の1615年）ではなく **げんわ**。',
    '  本をまたぐ読みの割れは validate_readings.py の検査7・8 が落とす',
    '- **読めても意味の分からない語（「奇貨居くべし」「仲父」）は地の文で意味が通るように書く。**',
...
function revisePrompt(id, frag, target, diffs, rest) {
  return withNote([
    ...
    `4. python3 scripts/reapply_profile_ruby.py ${frag} --write`,
    '   （足した文にもルビが要る。全出現へ機械が写す）。',
    '   **足す文でも訓読調を書かない**（崩じた→死んだ／賜った→与えた／請う→願い出た）—',
    '   列伝から持ち帰った場面ほど原文の言い回しに引きずられる',
    `5. python3 scripts/check_profile_fragment.py ${frag} --strict を通す`,
