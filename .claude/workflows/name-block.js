...
const LENSES = {
  ...
  glyph: '**字だけ**を見る。(1) 引用が底本の字体のまま残っているか（新字体・繁体へ' +
         ...
         '揃っているか、(3) **底本が字を私用領域（PUA）へ置いていないか**。' +
         '端末も `grep` も PUA を素通りするので**脱字に見えるが在る**（北魏太武帝の「神䴥」は' +
         'U+E154・西夏書事の「祐」は U+E3D7）。**見た目で「底本が字を落としている」と' +
         '判定せず** `[hex(ord(c)) for c in ...]` で確かめること。欠けたまま推測で補っていないかも見る',
}
...
const results = await pipeline(
  ...
  (id) => agent(
    ...
    `- **候補が元号ですらないことがある**（2026-08-17 実測）。絞り込みは2字の連なりを拾うだけなので、\n` +
    `  **尊号・国号・官名・定型句の断片**が候補に混ざります（遼太祖の「大聖」「應天」は\n` +
    `  「上尊号曰大聖大明天皇帝／后曰應天大明地皇后」の尊号・清太宗の「大清」は国号・\n` +
    `  「寬溫」は尊号・元成宗の「中書」は官名・「赦天」は「大赦天下」の切れ端）。\n` +
    `  しかも**これらは隣接ゲートを素通りします**（「曰大聖」が \`曰\` の隣接形に当たる）。\n` +
    `  **落とした候補の種別を builtOrDropped に必ず書く**（例:「大聖＝尊号なので落とした」）\n` +
    `- **改元条の見分け方は語順**: 「大赦建元神冊」「定有天下之号曰大清、改元崇德」のように、\n` +
    `  \`改元／建元\` の**直後に来る語**が建った元号です。同じ条に尊号・国号が並ぶ書\n` +
    `  （遼史・清史稿の即位条は上尊号と改元を1条に詰める）では、**近いだけの語を採らない**。\n` +
    `  evidence には改元の語と元号が繋がった形と、その条の**紀年**（「神冊元年春二月」等）を残すこと\n` +
    ...
    `- **底本が字を私用領域（PUA）へ置いていることがある。** 端末も \`grep\` も PUA を素通りするので\n` +
    `  **脱字に見えるが在る**。**見た目で「底本が字を落としている」と判定しない**\n` +
    `  （\`[hex(ord(c)) for c in ...]\` で確かめる）。**PUA を落とした形を「」に入れない**。\n` +
    `  それでも字が確定できないときだけ verdict を \`pending\` にし、evidence にコードポイントを書く\n` +
    `- **五代十国（\`wudai-*\`・\`shiguo-*\`）に入るときの注意**（2026-08-17 実測）:\n` +
    `  (a) **3書とも底本は簡体**（china-history の 旧五代史・新五代史、daizhige の 十国春秋。\n` +
    `  実測「国」1,157件・「國」0件）。**記憶で決めず着手時に1語 grep して確かめる**\n` +
    `  （2026-08-17 に「清史稿は繁体」と書いて3体に反証された）\n` +
    `  (b) **十国は本紀が立たない**ので \`daizhigev20/史藏/载记/十国春秋.txt\` を引く。\n` +
    `  **同じコーパスの \`史藏/别史/三十国春秋辑本.txt\` は十六国期の別の書**なので取り違えない\n` +
    `  (c) **十国のキャッシュは5〜32行と短く、改元条が入っていないのが常態**。\n` +
    `  本人のキャッシュで当たらないことを \`read-absent\` の根拠にせず、十国春秋・資治通鑑・\n` +
    `  新五代史の世家まで行くこと（それでも決まらなければ \`pending\`）\n` +
    `  (d) china-history には \`旧五代史-白话\` のような**白話訳の並び**がある。\n` +
    `  **原文ラベルでも中身が白話訳のことがある**（CORPUS_NOTES.md）ので、引用する前に1行見る\n` +
    `  (e) 五代十国は**避諱と PUA の記録がある**（\`docs/process/SOURCE_MAPPING.md\`）。\n` +
    `  字が欠けて見えたら \`[hex(ord(c)) for c in ...]\` で確かめる\n` +
    `- **北魏（\`beiwei-*\`）に入るときの注意**（2026-08-17 実測）:\n` +
    `  (a) 太武帝の元号「神䴥」の2字目は **U+E154**（daizhige・china-history あわせて103ファイル・332件。\n` +
    `  神□中/元/三/二/四 が167件で、残りは 白□鹿・麏□ など鹿の文脈）。**壊れてはいない**。\n` +
    `  北史は代用字「神蒨」を14件持つが**同じ書に PUA 形も6件併存する**ので代用であって\n` +
    `  独立の証人にならず（CORPUS_NOTES.md にも在る）、䴥 か 麚 かは**ローカルでは決まらない**。\n` +
    `  ここは \`pending\` で返し、\n` +
    `  evidence に「U+E154・両コーパスとも同じ PUA」と書くのが正解（推測で埋めない）\n` +
    `  (b) 北魏は**帝紀が立つ**ので tier は \`own-annals\`（検証1体）。ただし末期の短命な帝\n` +
    `  （元曄・元朗・元顥・元法僧…）は独立した紀を持たず、条が他帝の紀や列伝に在ることがある。\n` +
    `  **本人のキャッシュで当たらないときは「無い」ではなく、どの紀に在るかを探す**\n` +
    `  (c) 北魏の元号は**2字とは限らない**（太平真君・太延・正平…）。絞り込みの2字候補に引きずられない\n` +
    `- **遼・元・清（\`liao-*\`・\`yuan-*\`・\`qing-*\`）に入るときの注意**（2026-08-17 実測）:\n` +
    `  (a) **3書とも底本は簡体**（遼史・元史は china-history、清史稿は\n` +
    `  \`daizhigev20/史藏/正史/清史稿.txt\`。**daizhige だから繁体とは限らない** — 実測で\n` +
    `  「宣统」471件・「宣統」0件。2026-08-17 にこの一文が繁体と書いてあり、調査エージェント3体が\n` +
    `  別々に反証した）。**書ごとの字体は記憶で決めず着手時に1語 grep して確かめる**。\n` +
    `  引用は底本のまま・保存値は日本語の新字体（例「顺治」→「順治」・「崇德」→「崇徳」）\n` +
    `  (a2) **清の即位建元は改元・建元の語を使わない** — 「以明年為〈元号〉元年」（宣統）・\n` +
    `  「定年号」「詔改〈旧〉為〈新〉」（同治）の形なので、既定の走査語だと他人の元号しか当たらない\n` +
    `  (b) **「至元」は世祖（1264）と順帝（1335・後至元）で二度立つ**。grep すると\n` +
    `  大半が世祖の紀に落ちるので、**条の紀年で人物を分ける**（evidence に紀年を必ず書く）\n` +
    `  (c) **捨てた側が「新しく建った元号」のこともある** — 清穆宗は祺祥と建ててから\n` +
    `  「詔改祺祥為同治」で捨てている。「捨てた側＝前代の元号」と決めてかからない\n` +
    `  (d) **正史の範囲外の改元がある** — 宣統帝の3期目（1934・満洲国）は清史稿が扱わない。\n` +
    `  コーパスに条が無いなら \`read-absent\` ではなく \`pending\`（走査した書と範囲を evidence に書く）\n` +
    `- **西夏（\`xixia-*\`）に入るときの底本の穴**（2026-08-17 実測。CORPUS_NOTES.md にも在る）:\n` +
    `  (a) キャッシュの底本 \`daizhigev20/史藏/别史/西夏书事.txt\` は**珍しい字を私用領域（PUA）に置く**\n` +
    `  （100種。祐＝U+E3D7 が84件・鄜＝U+E24E が227件…）。**端末では消えて「脱字」に見えるが在る**\n` +
    `  ので、\`cat\`／Read の見た目だけで「底本が字を落としている」と判定しないこと\n` +
    `  （\`[hex(ord(c)) for c in ...]\` で確かめる）。**PUA を落とした形を「」に入れない**\n` +
    `  (b) 「奲」は daizhige では \`<奢单>\` という分解表記（PUA ではなく ASCII の記法）。\n` +
    `  **china-history 側の宋史 夏国伝は清字の「奲」を持つ**\n` +
    `  (c) **元号の並びと字数の権威は宋史 夏国伝の崩御条の列挙**\n` +
    `  （\`china-history/宋史/列传/第二百四十四章-卷二百四十四-原文.html\`＝夏国伝上・毅宗、\n` +
    `  \`…第二百四十五章-卷二百四十五-原文.html\`＝夏国伝下・恵宗・崇宗・仁宗。**繁体**）。\n` +
    `  「在位N年，改元A n年，B n年，…」の形で**建てた側だけ**が順に並ぶので、\n` +
    `  件数・順序・字数の突き合わせに使える。**値そのものは西夏書事の grep 出力から取る**\n` +
    `  (d) **短い候補は切り詰めかもしれない** — 「礼盛国慶」は「天賜礼盛国慶」の後ろ4字で、\n` +
    `  \`X元年\` の部分一致だとゲートを素通りする。列挙と字数を必ず突き合わせること\n` +
    `  (e) **キャッシュの行範囲は隣の皇帝と重ねてある**ので、名が当たっても**その人の元号とは限らない**\n` +
    `  （天儀治平は崇宗の最初の元号だが恵宗のキャッシュにも入る）。帰属は列挙で決める\n` +
