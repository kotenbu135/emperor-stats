...
| 欄 | 中身 |
|---|---|
| `ethnicName.kind` | 民族名の種類。`meta.catalogs.ethnicNameKinds` の ID（`khitan`／`jurchen`／`mongol`／`manchu`） |
...
「まだ分けていない・調べていない」を含みます。**西夏のタングート名・北魏の鮮卑名は
データにありません**（推測で補わないこと）。
...
| 欄 | 中身 |
|---|---|
| `name.posthumousName` | **通用する短縮呼称**（「文帝」「高皇帝」）。原典が短縮形を与えない政権では `null` のまま |
...
- **字体は新字体ですが、`hanzi_norm` の差分表に無い字は底本の字体のまま置きます。**
  `寛`↔`寬` が表に無く、`寛` と書くと `norm_for_match` がそのまま残すため底本の `宽` に
  当たりません。明穆宗・明宣宗の2件は `寬` で保存しています（表の穴そのものは残量表の行）
...
- **年は任意です。** 原文がその段に紀年を与えている場合だけ書きます
  （初諡は崩御条の中に在り、条そのものは年を名乗らないことが多い）。
  **並び順に対して逆行してはいけません** — この欄が既存の2欄に足しているのは順序だけなので、
  順序が壊れていたら欄そのものが嘘になります
- **冒頭形が段のどれとも一致しない人物が居ます。** 舊唐書は高宗の崩御条を
  「天皇大弘孝皇帝」、冒頭を「天皇大聖大弘孝皇帝」と書いており、大聖の2字が同じ書の中で
  食い違います（文宗も孝の1字）。**書の内部差なので、理由を書いた人物だけを通します**
  （`validate_emperors.py` の `POSTHUMOUS_STAGE_FULL_MISMATCH`）。黙って通すと
  転記ミスと区別できなくなります
- **底本照合は段ごとに数えます。** 人物単位で数えると、3段のうち1段が捏造でも
  「当たった人物」に入ってしまい、この欄でいちばん危ない失敗（在りもしない段を並べる）が
  見えません。2026-08-10 時点で**唐6人・延べ13段が13件とも本人の原文に当たります**
