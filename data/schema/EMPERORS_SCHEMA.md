...
契丹名として伝わるのは**小字の側**です。小字だけが在る人物は14人いて、この欄は空のままです。
...
| B 名乗りの分離 | 諱・民族名・廟号・諡号のどれかと同じ値（別の名乗りを写した形） | 同上 |
...
- **底本側の事情で当たらない1件は `COURTESY_ALLOW` に理由付きで挙げ**、実行のたびに
  ...
  daizhige の三国志は「字元仲」）
- **サイトの画面には出していません。** 名前チップに出すには92件ぶんのふりがなが要り、
  `rubyOf` が未登録の漢字表示文字列で落ちるためで、配布 CSV の `courtesyName` 列が
  いまのところ唯一の出口です
