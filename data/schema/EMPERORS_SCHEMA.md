...
| `dynastyOrder` | number \| null（**欄自体が無いこともある**） | 王朝自身が数える歴代の通し番号（復位も別カウント）。**その政権自身の継承順**を歴代とし、皇帝を称さなかった君主（前涼の涼王・北周の孝閔帝＝天王など）も番号に含めるため、365人の収録者だけを数えた値とは一致しないのが普通。証人の優先順位は (1) その政権自身の正史の帝紀（隋書・両唐書・宋史・遼史・金史・元史・明史・清史稿・旧五代史）、(2) 準正史の紀（十国＝十国春秋の本紀20巻・南明＝小腆紀傳の紀第一〜七・北元＝新元史の本紀）、(3) 紀が1つも無い政権（西夏・西遼・隋末群雄・元末群雄・安史燕・呉周・中華帝国など47人）は列伝・編年（西夏書事・資治通鑑）の継承記述、の順（2026-08-20 のユーザー決定・Issue #24）。**紀が無いことを「歴代に数えない」と読み替えない** — `null` を置くのは、その政権自身の継承に入らない並立・僭称の在位だけ。**番号を消費するかどうかの線は「実際に位に即いて統治したか」**で、追尊されただけの祖先は消費しません（後唐の朝廷自身が「承乾御宇之君，行事方云實錄；追尊册號之帝，約文只可紀年」と実録／紀年録で分け、太祖 李克用以上を後者に置いています＝旧五代史 明宗紀六）。**「追尊だから消費しない」とは書けません** — 元の太祖〜憲宗は諡号も廟号も没後40〜80年の追尊ですが、実際に統治したので1〜4を消費し `yuan-shizu` は5です。**本データ365人に収録が無い君主が並びに入ると欠番になります**（清の第1代ヌルハチ・元の第1〜4代・南漢の第1代劉隱・呉の第1〜3代・閩の第1〜3代）。**欄の在り方が3値で意味を分けます**（2026-08-03・Issue #69。`validate_emperors.py` の `check_dynasty_order` が検査）: 数値＝第N代／`null`＝**調べた上で歴代に数えない**在位（宋の元凶劭・梁の蕭正徳・北魏の南安王余など、帝紀を立てられていない僭称・並立。**21在位**）／**欄が無い**＝その政権が未調査（`meta.catalogs.regimes[].dynastyOrderSurveyed: false` の政権）。**2026-08-20 に 89/89政権を読み切ったので「欄が無い」は現在 0 件**（374在位すべてに欄が在り、数値353・`null` 21）だが、**意味は残す** — 政権を新しく足したときはまず `dynastyOrderSurveyed: false`＋欄なしから始まり、`check_dynasty_order` が双方向で検査する。移行前は `null` が未調査と該当なしを兼ねていて、レコード単体からは区別できませんでした。**未調査ぶんを在位順から機械導出して埋めない**（導出値は主張ではありません）。番号は**政権カタログの単位**で振るので、旧唐書が則天皇后本紀を唐の本紀の並びに立てていても**武周は独立政権として第1代＝1**とし、唐の通し番号は則天を飛ばして中宗→睿宗と続きます（2026-08-20 のユーザー決定・Issue #24） |
...
**2026-08-13 に転記を済ませました**（**20セル・15人**。当日の1巡目で 19セル・14人を入れ、
同日の2巡目で**金 海陵王の王諡「煬」1人ぶんの取りこぼし**が見つかって足しました。
同じ金史の衛紹王「紹」とは構造が同じで、残量表も2件一緒に決めると書いていたのに片方だけ入れていました）。段の欄 `posthumousNames` のゲートCは
「皇帝」「大帝」で結ぶ形しか通さないので、**この5型はすべて `POSTHUMOUS_STAGE_NON_IMPERIAL` に
1件ずつ理由を書いて通します**（形の資格を緩めたのではなく、例外を名指しで数えられる形にしてあります）。
**形は条が書いている形をそのまま採ります** — 底本照合（ゲートF）が本人のキャッシュに
連続一致を見るので、隋 煬帝へ皇泰主が贈った「明」を「明皇帝」へ伸ばすと空振りします。
**決定が外したのは形の資格だけで、証人の要否は変わりません** — 前涼 張祚の王諡「威王」は
晉書 載記の本人の条に無いので、決定後も段は空（`read-absent`）のままです。
**贈られた諡に廟号が付いてくる場合、廟号は欄へ入れません**（侯景が梁 簡文帝へ贈った「庙称高宗」、
王世充が隋 恭帝侗を廃したのちの記事など。決定の範囲は諡だけです）。
...
**2026-08-13 に遼史・元史を同じ形で打ち切りました**（遼8人・元8人）— どちらも先帝を
「〈廟号〉皇帝」か全長諡で指し、諡の末字＋皇帝の形を1件も使いません。
**遼太祖の「天皇帝」は諡の短縮形として扱いません**（初出が「建天皇帝名号」＝生前の尊号で、
崩御後の諡は「升天皇帝」→「大聖大明天皇帝」→「大聖大明神烈天皇帝」といずれも全長形。
全長諡の末3字と同形なのは諡が尊号の上に立てられたためです）。
**諡そのものが無い人物はこの打ち切りの例外**（遼 天祚帝・元 泰定帝・天順帝・恵宗）で、
空の理由が「書が短縮形を使わない」ではなく「諡が無い」なので、断片側の `read-absent` で受けます。
...
- **「皇帝」で結ばない諡が在ります。** 明代宗は郕王へ落とされて王諡「戾」を与えられ、
  ...
  **1つの書の中で形が3型に割れることがあります**（2026-08-13・新五代史 十国世家）— 全長形6人・
  **諡字1字**2人（楊溥「谥曰睿」・劉玢「谥曰殇」）・**廟号型2字**2人（閩が2代続けて
  「谥昶曰康宗」「谥曰景宗」と書く）が同じ書に同居します。**書の慣行として一方に寄せない**
  のがここでの読み方で、条が書いている形をそのまま採ります
- **「尊号」の語で書かれた諡が在ります。** 漢書は帝の諡を授ける条に「谥」の字を使わず、
  高帝紀下が葬送のあとに「上尊号曰高皇帝」と結びます。**生前の尊号と分けるのは語ではなく位置**で、
  崩御・葬送のあとに贈られた形なら段に置きます（逆に、楊溥の在位中の「加尊号睿聖文明孝皇帝」は
  同じ世家に在っても段ではありません）
