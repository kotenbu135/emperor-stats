...
**単位ごとの追い足しはサブディレクトリへ置く。** 直下の `<皇帝id>.json` は
「その人物の名前欄をひと通り確定した回」の断片なので、後から別の欄を足した回で
上書きしない。`posthumous-stages-2026-08-10/`（諡号の段・唐21人＋明16人）がこれで、
`check_claims.py` の `*.json` 走査は再帰しないので**そのディレクトリを指して**流す。

```bash
python3 scripts/check_claims.py data/internal/name-fragments/posthumous-stages-2026-08-10/
```
